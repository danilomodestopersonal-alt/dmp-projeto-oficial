...
function MiniMonthCalendar({onSelect,events}:{onSelect:(date:string)=>void;events:KidsEvent[]}){
  ...
  function safeEventRange(event:KidsEvent){
    let startDate=event.startDate;
    let endDate=event.endDate||event.startDate;

    // O evento oficial da clínica é de um único dia (13/09/2026).
    // Esta proteção evita que um intervalo antigo/sujo pinte a semana inteira.
    if(event.id==="event-clinica-ricardo-2026"){
    ...
    let guard=0;
    while(current<=finish&&guard<370){
  ...
  events.forEach(event=>{
    safeEventRange(event).forEach(value=>{
    ...
    <div className="mini-month-days">
      ...
      {Array.from({length:days},(_,index)=>{
        ...
        return <button
          key={day}
          className={className}
          aria-label={hasEvent?`${day}: evento`:`${day}`}
          title={hasEvent?(eventDates.get(value)||[]).map(item=>item.name).join(" · "):undefined}
    ...
    {selectedDate&&selectedEvents.length?
      <div className="mini-month-event-popover">
        <div className="mini-month-event-head">
          ...
          <button onClick={()=>setSelectedDate(null)} aria-label="Fechar">×</button>
        </div>
        {selectedEvents.map(event=><button
          key={event.id}
          className="mini-month-event-item"
