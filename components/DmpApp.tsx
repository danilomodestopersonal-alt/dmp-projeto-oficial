        ...
        <section className="content student-profile-page">
          <div className="hero student-profile-hero">
            ...
            <div className="hero-actions">
              {workoutEntries.length ? <button className="primary" onClick={() => setTab("workouts")}>â–¶ Usar treino montado</button> : null}
              <button className={workoutEntries.length ? "secondary" : "primary"} onClick={() => setView("free-session")}>ðŸŽ¤ Registrar o que fez</button>
              <button className="secondary" onClick={() => setView("attendance-session")}>âœ“ SÃ³ presenÃ§a</button>
              <button className="secondary" onClick={() => setShowEditStudentForm(true)}>Editar aluno</button>
              <button className="secondary" onClick={()=>void printPersonalStudentReport(selectedStudent)}>ðŸ–¨ï¸ RelatÃ³rio</button>
              <button className="secondary" onClick={toggleArchive}>{selectedStudent.status === "ACTIVE" ? "Marcar inativo" : "Reativar"}</button>
              {whatsappLink(selectedStudent.phone)?<a className="secondary button-link whatsapp-student-link" href={whatsappLink(selectedStudent.phone)!} target="_blank" rel="noreferrer">ðŸŸ¢ WhatsApp</a>:null}
            </div>
          </div>
          ...
          <nav className="profile-tabs">
            {(["summary","workouts","history","assessments"] as StudentTab[]).map(item => (
              <button key={item} className={tab === item ? "active" : ""} onClick={() => setTab(item)}><span>{item==="summary"?"ðŸ“‹":item==="workouts"?"ðŸ‹ï¸":item==="history"?"ðŸ•˜":"ðŸ“"}</span>{tabLabel(item)}</button>
            ))}
          </nav>
...
function AddStudentsToCalendarEventModal({event,students,onClose,onSaved}:{event:CalendarEvent;students:Student[];onClose:()=>void;onSaved:()=>void}){
  ...
  async function save(){if(!selected.length)return;setSaving(true);const names=students.filter(student=>selected.includes(student.id)).map(calendarStudentDisplayName);const summary=`${event.summary} ${names.join(" ")}`.replace(/\s+/g," ").trim();const response=await fetch("/api/google/events",{method:"PATCH",headers:{"content-type":"application/json"},body:JSON.stringify({id:event.id,summary,description:event.description||"",location:event.location||"",start:event.start,end:event.end})});setSaving(false);if(response.ok)onSaved();else alert("NÃ£o foi possÃ­vel adicionar os alunos ao compromisso.");}
  return <div className="modal-backdrop"><section className="modal"><div className="modal-head"><div><h2>Adicionar alunos</h2><p className="muted">{event.summary} Â· escolha um ou vÃ¡rios alunos.</p></div><button className="text-button" onClick={onClose}>Fechar</button></div><input className="search" value={search} onChange={e=>setSearch(e.target.value)} placeholder="Buscar aluno..."/><div className="calendar-student-picker">{available.map(student=><button type="button" key={student.id} className={selected.includes(student.id)?"selected":""} onClick={()=>toggle(student.id)}><span>{selected.includes(student.id)?"âœ“":"+"}</span><strong>{student.name}</strong></button>)}</div>{!available.length?<div className="calendar-empty">Nenhum aluno disponÃ­vel nesta busca.</div>:null}<div className="modal-actions"><button onClick={onClose}>Cancelar</button><button className="primary" disabled={!selected.length||saving} onClick={save}>{saving?"Salvando...":`Adicionar ${selected.length||""}`}</button></div></section></div>;
}
...
function Sidebar({current,onNavigate,logout}:{current:View;onNavigate:(view:View)=>void;logout:()=>void}) {
  ...
  return <aside className="dashboard-sidebar"><div className="dashboard-logo-card"><img src="/logo-danilo.jpg" alt="Danilo Modesto Personal Trainer" className="dashboard-sidebar-logo" /></div><nav className="dashboard-nav">{orderedItems.filter(item=>mobile||item.view!=="kids").map(item=><button key={item.view} className={`dashboard-nav-item ${current===item.view?"active":""}`} onClick={()=>onNavigate(item.view)}>{item.icon} {item.label}</button>)}</nav>{!mobile?<button className={`sidebar-kids-special ${current==="kids"?"active":""}`} onClick={()=>onNavigate("kids")}><img src="/logo-ctds.png" alt="CT DS Tennis"/><span><small>GestÃ£o pedagÃ³gica</small><strong>Aulas Kids</strong></span></button>:null}{!mobile?<button className="dashboard-logout" onClick={logout}>Sair</button>:null}{!mobile?<span className="sidebar-resize-handle" onPointerDown={event=>beginPanelResize(event,"sidebar")}/>:null}</aside>;
}
    ...
    <aside className="desktop-agenda-rail">
      ...
      <div className="agenda-rail-scroll">
        ...
        {upcoming.length?upcoming.map(event=>{
          ...
          previousDate=date;
          const dayLabel=date===todayKey?"Hoje":date===tomorrowKey?"Amanh?":"Dia seguinte";
...
function tabLabel(tab:StudentTab){return({summary:"Resumo",workouts:"Treinos",history:"HistÃ³rico",assessments:"AvaliaÃ§Ãµes"})[tab];}
