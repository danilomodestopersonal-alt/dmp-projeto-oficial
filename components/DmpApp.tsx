...
const [cloudWritable, setCloudWritable] = useState(false);
  ...
  // MantÃ©m uma entrada de histÃ³rico interna para o botÃ£o/gesto Voltar do Android.
  useEffect(() => {
    ...
    const onPopState=()=>{
      ...
      if(view==="workout-editor"&&!confirm("Voltar sem salvar? AlteraÃ§Ãµes feitas nesta montagem podem ser perdidas.")){rearm();return;}
      ...
      // Em Hoje, nÃ£o rearma: um novo Voltar pode sair normalmente.
    };
  ...
  async function loadStudentsFromCloud() {
    ...
    try {
      ...
      if (!Array.isArray(result.data)) {
        throw new Error("Dados da nuvem invÃ¡lidos");
      }
      ...
      console.error("Nuvem indisponÃ­vel; usando backup local:", error);
...
fetch("/api/google/status").then(r=>r.json()).then(setCalendarStatus).catch(()=>{});
  ...
  function deleteSelectedStudent() {
    ...
    if(!confirm("Esta aÃ§Ã£o apaga o cadastro, treinos, histÃ³rico e avaliaÃ§Ãµes deste aluno. Confirmar exclusÃ£o definitiva?"))return;
  ...
  function clearWorkout(workout:Workout) {
    if (!selectedStudent || !confirm(`Limpar o Treino ${workout.slot || ""}? O histÃ³rico das aulas serÃ¡ preservado.`)) return;
  ...
  function archiveWorkout(workout: Workout) {
    ...
    const confirmed = confirm(`Arquivar o Treino ${slot}? A ficha ficarÃ¡ em Treinos arquivados e a aba ${slot} serÃ¡ liberada. O histÃ³rico das aulas nÃ£o serÃ¡ alterado.`);
    if (!confirmed) return;
  ...
  function copyWorkoutToStudent(targetStudentId:string,targetSlot:WorkoutSlot) {
    ...
    if(collision&&!confirm(`${target.name} jÃ¡ possui o Treino ${targetSlot}. Substituir essa ficha pela cÃ³pia?`))return;
    const copy:Workout={
      ...workoutToCopy,
      id:crypto.randomUUID(),
      slot:targetSlot,
  ...
  function duplicateWorkout() {
    ...
    if (!nextSlot) {
      alert("As quatro abas A, B, C e D jÃ¡ possuem treino.");
      return;
    }
  ...
  async function saveSession(session: Session) {
    ...
    try {
      ...
      alert("NÃ£o foi possÃ­vel salvar esta sessÃ£o. Confira a conexÃ£o e tente novamente.");
    }
  }
  ...
  function resetData() {
    if (!confirm("Restaurar os dados importados de 2026? AlteraÃ§Ãµes locais serÃ£o apagadas.")) return;
    resetImportedData();
  ...
  async function registerAbsence(student:Student,event:CalendarEvent){
    ...
    if(!response.ok){alert("NÃ£o foi possÃ­vel atualizar a agenda Google. A ausÃªncia nÃ£o foi salva.");return;}
  ...
  async function registerStudentAbsence(student:Student){
    ...
    const absence:Session={id:crypto.randomUUID(),date:today(),workoutName:"AusÃªncia",notes:"AusÃªncia informada pelo cadastro do aluno.",completedExercises:[],source:"ABSENCE",finishedAt:new Date().toISOString()};
    updateStudentRecord({...student,sessions:[absence,...student.sessions]});
  }
  ...
  {spotifyState.connected ? <>
    ...
    <div className="spotify-controls">
      <button type="button" disabled={spotifyBusy} onClick={()=>void controlSpotify("previous")} aria-label="MÃºsica anterior">â—€</button>
      <button type="button" disabled={spotifyBusy} onClick={()=>void controlSpotify(spotifyState.isPlaying?"pause":"play")} aria-label={spotifyState.isPlaying?"Pausar":"Tocar"}>{spotifyState.isPlaying?"âšâš":"â–¶"}</button>
      <button type="button" disabled={spotifyBusy} onClick={()=>void controlSpotify("next")} aria-label="PrÃ³xima mÃºsica">â–¶</button>
    </div>
            ...
            <div className="mobile-header-actions"><button className="mobile-header-logout" onClick={logout}>Sair</button><div className="mobile-action-stack"><button className="mobile-quick-launch" onClick={()=>setShowMobileActions(true)} aria-label="Abrir aÃ§Ãµes rÃ¡pidas">ï¼‹</button><button className="mobile-voice-launch" onClick={()=>{sessionStorage.setItem("dmp_finance_voice_start","1");navigateMain("finance");}} aria-label="Falar lanÃ§amento financeiro">ðŸŽ¤</button></div></div>
          ...
          {view === "students" ? <>
            <header className="dashboard-topbar"><div><p className="dashboard-eyebrow">Painel de atendimento</p><h1>Alunos</h1><p>Cadastros, fichas, observaÃ§Ãµes, restriÃ§Ãµes e histÃ³rico.</p></div><div className="hero-actions"><button className="secondary" onClick={resetData}>Restaurar importaÃ§Ã£o</button><button className="primary" onClick={() => setShowStudentForm(true)}>+ Novo aluno</button></div></header>
...
{(()=>{
  ...
  const ageGroups=[
    ["AtÃ© 30",ages.filter(age=>age<=30).length],
    ["31â€“40",ages.filter(age=>age>=31&&age<=40).length],
    ["41â€“50",ages.filter(age=>age>=41&&age<=50).length],
    ["51â€“60",ages.filter(age=>age>=51&&age<=60).length],
    ["60+",ages.filter(age=>age>60).length]
  ] as const;
  return <>
    <Stat icon="ðŸ‘¥" label="Alunos ativos" value={active.length}/>
    <Stat icon="ðŸ“" label="Alunos inativos" value={inactive.length}/>
    <article className="stat-card student-demographic-card"><span className="stat-icon">ðŸ‘¨ðŸ‘©</span><div><small>Homens / Mulheres</small><strong>{men} / {women}</strong><em>{genderStudents.length} de {active.length} cadastrados</em></div></article>
    <article className="stat-card student-demographic-card age-card"><span className="stat-icon">ðŸŽ‚</span><div><small>Faixa etÃ¡ria</small><div className="age-stat-list">{ageGroups.map(([label,value])=><span key={label}><b>{label}</b><strong>{value}</strong></span>)}</div><em>{ages.length} de {active.length} cadastrados</em></div></article>
  </>;
})()}
</div>
  ...
  return <><header className="dashboard-topbar"><div>
    ...
    <p>Veja quem treinou, a frequÃªncia do mÃªs e quais alunos estÃ£o hÃ¡ mais tempo sem registro.</p>
  </div></header>

  <section className="dashboard-content">

    <div className="dashboard-stats four-stats">
      <Stat icon="ðŸ‹ï¸" label="Treinos no mÃªs" value={monthWorkouts}/>
      <Stat icon="ðŸ‘¥" label="Alunos treinados" value={trainedThisMonth}/>
      <Stat icon="ðŸ“Š" label="MÃ©dia por aluno" value={average}/>
      <Stat icon="âš ï¸" label="Sem treino 14+ dias" value={withoutRecent}/>
    </div>

    {withoutRecent>0?<div className="training-attention-banner">
      <strong>âš  {withoutRecent} aluno{withoutRecent===1?"":"s"} precisam de atenÃ§Ã£o</strong>
      <span>Sem treino registrado hÃ¡ 14 dias ou mais, ou ainda sem nenhum treino registrado.</span>
    </div>:null}
    ...
    <div className="overview-list training-control-list">
      {ordered.map(item=>{
        ...
        const slots=item.entries.map(entry=>entry.slot).join(" Â· ");
        ...
        >
          <span>
            ...
            <small>
              {item.last
                ? <>Ãšltimo treino: <b>{formatDate(item.last.date)}</b> Â· {item.last.workoutName||"Treino registrado"}</>
                : <>Nenhum treino registrado</>}
            </small>
            <small className="training-month-line">
              {item.monthSessions.length} treino{item.monthSessions.length===1?"":"s"} neste mÃªs
              {item.entries.length?<> Â· ðŸ“‹ Ficha {slots}</>:null}
            </small>
          </span>
          ...
          {view === "history-overview" ? <><header className="dashboard-topbar"><div><p className="dashboard-eyebrow">Linha do tempo</p><h1>HistÃ³rico</h1><p>Pesquise qualquer sessÃ£o por aluno, exercÃ­cio, observaÃ§Ã£o ou tipo de registro.</p></div></header><section className="dashboard-content"><div className="history-toolbar"><input className="search" placeholder="Buscar aluno, exercÃ­cio ou observaÃ§Ã£o..." value={historySearch} onChange={e=>setHistorySearch(e.target.value)}/><select value={historySource} onChange={e=>setHistorySource(e.target.value as any)}><option value="ALL">Todos os tipos</option><option value="PLANNED">Ficha concluÃ­da</option><option value="FREE">Treino registrado</option><option value="ATTENDANCE">PresenÃ§a</option><option value="IMPORTED">Importado</option></select><span className="status-chip ok">{filteredHistory.length} registro{filteredHistory.length===1?"":"s"}</span></div><div className="overview-list">{filteredHistory.slice(0,500).map(({student,session})=><button className="overview-row" key={session.id} onClick={()=>openStudent(student.id)}><span><strong>{formatDate(session.date)} Â· {student.name}</strong><small>{session.workoutName}{session.notes?` â€” ${session.notes}`:""}</small></span><span className="status-chip ok">{sessionSourceLabel(session)}</span></button>)}</div></section></> : null}
  ...
  return <><header className="dashboard-topbar"><div>
    <p className="dashboard-eyebrow">Controle de avaliaÃ§Ãµes</p>
    <h1>Acompanhamento dos alunos</h1>
    <p>Veja rapidamente quem estÃ¡ em dia e quem precisa de uma nova avaliaÃ§Ã£o.</p>
  </div></header>

  <section className="dashboard-content">

    <div className="dashboard-stats four-stats">
      <Stat icon="ðŸŸ¢" label="Em dia" value={ok.length}/>
      <Stat icon="ðŸŸ¡" label="PrÃ³ximas do prazo" value={soon.length}/>
      <Stat icon="ðŸ”´" label="Vencidas" value={expired.length}/>
      <Stat icon="âšª" label="Sem avaliaÃ§Ã£o" value={none.length}/>
    </div>
    ...
    <div className="assessment-control-summary">
      ...
      <span>avaliaÃ§Ãµes realizadas neste mÃªs</span>
    </div>
    ...
    <div className="overview-list assessment-control-list">
      {ordered.map(item => {
        const label =
          ...
          item.status === "SOON" ? "PrÃ³xima do prazo" :
        ...
        >
          <span>
            ...
            <small>
              {item.latest
                ? <>Ãšltima avaliaÃ§Ã£o: <b>{formatDate(item.latest.date)}</b> Â· {item.days} dia{item.days===1?"":"s"} atrÃ¡s</>
                : <>Nenhuma avaliaÃ§Ã£o cadastrada</>}
            </small>
          </span>
  ...
  if (view === "student") {
    ...
    const workoutSlots = workoutEntries.map(entry => entry.slot).join(" Â· ");
        ...
        <section className="content student-profile-page">
            ...
            <div className="student-profile-command-main">
              <div className="student-profile-command-identity">
                ...
                <div>
                  ...
                  <p>{selectedStudent.goal || "Objetivo nÃ£o informado"}</p>
                </div>
              </div>

              <nav className="student-profile-primary-tabs" aria-label="Ãreas do aluno">
                  ...
                  >
                    <span className="student-profile-tab-icon">
                      {item==="summary"?"ðŸ“‹":
                       item==="workouts"?"ðŸ‹ï¸":
                       item==="history"?"ðŸ•˜":
                       item==="assessments"?"ðŸ“":"ðŸ“"}
                    </span>

                    <span className="student-profile-tab-text">
                      <strong>{tabLabel(item)}</strong>
                      <small>
                        {item==="summary"?"VisÃ£o geral":
                         item==="workouts"?"Fichas e montagem":
                         item==="history"?"SessÃµes realizadas":
                         item==="assessments"?"EvoluÃ§Ã£o fÃ­sica":
                         "Documentos"}
                      </small>
                    </span>
                  </button>
          ...
          <section className={`workflow-strip ${workoutEntries.length ? "has-workout" : "no-workout"}`}>
            <div>
              ...
              <span>{workoutEntries.length ? "Escolha A, B, C ou D para acompanhar a aula. VocÃª tambÃ©m pode registrar por voz/texto ou somente presenÃ§a." : "Monte uma aba de treino ou registre a aula por voz/texto sem ficha."}</span>
            </div>
            <div className="workflow-strip-actions">
              ...
              <button className="secondary" onClick={() => setView("attendance-session")}>SÃ³ presenÃ§a</button>
            </div>
          </section>
          ...
          <div className="student-danger-zone"><button className="danger-link" onClick={deleteSelectedStudent}>Excluir cadastro</button><small>ExclusÃ£o definitiva do aluno e dos dados vinculados.</small></div>
        </section>
...
function CalendarTodayPanel({status,events,loading,sync,students,todaySessions,onOpenAgenda,onOpenStudent,onStartStudent,onAbsence,onOpenKids}:{status:{configured:boolean;connected:boolean};events:CalendarEvent[];loading:boolean;sync:{dailyAt:string;weeklyAt:string;weeklyCount:number};students:Student[];todaySessions:{student:Student;session:Session}[];onOpenAgenda:()=>void;onOpenStudent:(id:string)=>void;onStartStudent:(id:string,mode:"session"|"free"|"attendance")=>void;onAbsence:(student:Student,event:CalendarEvent)=>void;onOpenKids:(event:CalendarEvent)=>void}) {
  ...
  return <section className="panel calendar-today-panel"><div className="panel-head"><div><h2>Agenda de hoje</h2>{status.connected?<p className="calendar-auto-sync">â†» AtualizaÃ§Ã£o automÃ¡tica ativa{sync.dailyAt?` Â· Ãºltima ${formatSyncTime(sync.dailyAt)}`:""}</p>:null}</div><button className="secondary" onClick={onOpenAgenda}>Abrir agenda</button></div>
    {!status.configured ? <div className="calendar-empty"><strong>IntegraÃ§Ã£o pronta no aplicativo</strong><span>Falta apenas configurar as credenciais do Google para conectar sua agenda.</span></div> : !status.connected ? <div className="calendar-empty"><strong>Google Agenda ainda nÃ£o conectado</strong><span>Abra a aba Agenda e toque em â€œConectar Googleâ€.</span></div> : loading ? <div className="calendar-empty"><span>Carregando compromissos...</span></div> : displayEvents.length ? <div className="calendar-preview-list">{displayEvents.slice(0,10).map(event=>{const slotStudents=getCalendarEventStudents(event,students);const kids=kidsCalendarRequest(event);const allDone=slotStudents.length>0&&slotStudents.every(student=>completedIds.has(student.id)||absentIds.has(student.id));return <article key={event.id} className={`calendar-preview-row central-row calendar-multi-row ${allDone?"event-done":""}`}><span className="calendar-time">{formatCalendarTime(event)}</span><div className="calendar-slot-main"><button className="calendar-event-main" onClick={kids?()=>onOpenKids(event):onOpenAgenda}><strong>{event.summary}</strong><small>{kids?"Aula TÃªnis Kids":slotStudents.length?`${slotStudents.length} aluno${slotStudents.length===1?"":"s"} neste horÃ¡rio`:"Compromisso da agenda"}</small></button>{kids?<button className={`primary compact-action kids-action-${kids.category.toLowerCase()}`} onClick={()=>onOpenKids(event)}>ðŸŽ¾ Abrir turma e chamada</button>:slotStudents.length?<div className="calendar-slot-students"><span className="calendar-slot-title">Treinos do horÃ¡rio</span>{slotStudents.map(student=>{const done=completedIds.has(student.id);const absent=absentIds.has(student.id);const latestAssessment=student.assessments.slice().sort((a,b)=>b.date.localeCompare(a.date))[0];const assessmentAgeDays=latestAssessment?Math.floor((Date.now()-new Date(latestAssessment.date+"T12:00:00").getTime())/86400000):null;const assessmentExpired=assessmentAgeDays!==null&&assessmentAgeDays>60;const workoutEntry=getStudentWorkoutEntries(student)[0];const workoutValidity=workoutEntry?workoutValidityInfo(student,workoutEntry.workout):null;return <div className="calendar-slot-student" key={student.id}><span className="calendar-student-contact">
...
</span><span className={latestAssessment?(assessmentExpired?"home-assessment-badge expired":"home-assessment-badge ok"):"home-assessment-badge none"}>{latestAssessment?`ðŸ“ ${formatDate(latestAssessment.date)}`:"ðŸ“ Sem avaliaÃ§Ã£o"}</span>{workoutEntry?<button type="button" className={`home-workout-badge home-workout-badge-button ${workoutValidity?.status.toLowerCase()||"none"}`} onClick={()=>{window.open(`/app?mode=planned-session&student=${encodeURIComponent(student.id)}&workout=${encodeURIComponent(workoutEntry.workout.id)}`,"_blank","noopener,noreferrer");}} title={`Abrir Treino ${workoutEntry.slot} em nova janela`}><b>Treino {workoutEntry.slot}</b><small>{workoutValidity?.status==="NONE"?`${workoutEntry.workout.exercises.length} exercÃ­cios`:workoutValidity?.endDate?`atÃ© ${formatDate(workoutValidity.endDate)}`:workoutValidity?.label}</small></button>:null}{done?<span className="status-chip ok">âœ“ Finalizado</span>:absent?<span className="status-chip absent">Ausente</span>:null}{!done&&!absent?<span className="calendar-student-actions"><button className={workoutEntry?"secondary compact-action":"primary compact-action"} onClick={()=>onStartStudent(student.id,"free")}>âœ Registrar</button><button className="secondary compact-action" onClick={()=>onStartStudent(student.id,"attendance")}>âœ“ PresenÃ§a</button><button className="absence-action compact-action" onClick={()=>void onAbsence(student,event)}>AusÃªncia</button></span>:null}</div>})}</div>:null}</div><span className="status-chip">{calendarEventStatus(event)}</span></article>})}</div> : <div className="calendar-empty"><strong>Nenhum compromisso hoje</strong><span>Sua agenda Google estÃ¡ conectada.</span></div>}
  </section>;
}
...
function CalendarAgenda({status,events,loading,sync,students,range,anchor,onRange,onAnchor,onOpenStudent,onStartStudent,onOpenKids,onStatusChange,onRefresh,onNewEvent}:{status:{configured:boolean;connected:boolean};events:CalendarEvent[];loading:boolean;sync:{dailyAt:string;weeklyAt:string;weeklyCount:number};students:Student[];range:AgendaRange;anchor:string;onRange:(value:AgendaRange)=>void;onAnchor:(value:string)=>void;onOpenStudent:(id:string)=>void;onStartStudent:(id:string,mode:"session"|"free"|"attendance")=>void;onOpenKids:(event:CalendarEvent)=>void;onStatusChange:(value:{configured:boolean;connected:boolean})=>void;onRefresh:()=>void;onNewEvent:()=>void}) {
  ...
  async function removeEvent(id:string){if(!confirm("Excluir este compromisso do Google Calendar?"))return;const r=await fetch(`/api/google/events?id=${encodeURIComponent(id)}`,{method:"DELETE"});if(r.ok)onRefresh();else alert("NÃ£o foi possÃ­vel excluir o compromisso.");}
  ...
  return <>
    <section className="agenda-view-toolbar"><div>{(["day","week","month","year","list"] as AgendaRange[]).map(value=><button key={value} className={range===value?"filter-active":"secondary"} onClick={()=>onRange(value)}>{agendaRangeLabel(value)}</button>)}</div><div className="agenda-period-nav"><button className="secondary" onClick={()=>navigate(-1)} aria-label="PerÃ­odo anterior">â€¹</button><strong>{agendaPeriodLabel(anchor,range)}</strong><button className="secondary" onClick={()=>navigate(1)} aria-label="PrÃ³ximo perÃ­odo">â€º</button></div><input type="date" value={anchor} onChange={event=>onAnchor(event.target.value)}/><button className="primary" onClick={onNewEvent}>+ Novo compromisso</button></section>
    {!status.configured?<section className="panel setup-panel"><h2>Uma configuraÃ§Ã£o Ãºnica</h2><p>Para ativar, crie as credenciais OAuth no Google Cloud e configure <code>GOOGLE_CLIENT_ID</code>, <code>GOOGLE_CLIENT_SECRET</code> e <code>APP_URL</code>. Depois o mesmo login funciona no computador e no celular.</p></section>:null}
    ...
    <section className="panel agenda-connect agenda-connect-bottom"><div className="agenda-icon">ðŸ“…</div><div className="agenda-connect-main"><h2>Google Calendar</h2><p>O Google continua sendo a agenda oficial. O DMP mantÃ©m os nomes e horÃ¡rios como estÃ£o na sua agenda e abre os treinos dos alunos daquele horÃ¡rio.</p><div className="agenda-roadmap"><span>âœ“ Atualiza ao abrir/voltar ao DMP</span><span>âœ“ Revisa o dia a cada 5 min em uso</span><span>âœ“ Domingo: prÃ©-carrega 7 dias</span><span>âœ“ VÃ¡rios alunos no mesmo horÃ¡rio</span><span>âœ“ Criar/excluir compromisso pelo DMP</span></div>{status.connected?<div className="agenda-sync-summary"><strong>SincronizaÃ§Ã£o automÃ¡tica ativa</strong><span>Hoje{sync.dailyAt?` atualizado Ã s ${formatSyncTime(sync.dailyAt)}`:" aguardando primeira atualizaÃ§Ã£o"}.</span><span>{sync.weeklyAt?`Ãšltima revisÃ£o semanal: ${formatSyncDateTime(sync.weeklyAt)} Â· ${sync.weeklyCount} compromissos.`:"A revisÃ£o dos prÃ³ximos 7 dias acontece automaticamente no primeiro uso de domingo."}</span></div>:null}</div><div className="agenda-actions">{!status.configured?<span className="status-chip">ConfiguraÃ§Ã£o pendente</span>:status.connected?<><span className="status-chip ok">Conectado</span><button className="primary" onClick={onNewEvent}>+ Compromisso</button><button className="secondary" onClick={onRefresh}>Atualizar</button><button className="secondary" onClick={disconnect}>Desconectar</button></>:<a className="primary button-link" href="/api/google/auth">Conectar Google</a>}</div></section>
...
function AddStudentsToCalendarEventModal({event,students,onClose,onSaved}:{event:CalendarEvent;students:Student[];onClose:()=>void;onSaved:()=>void}){
  ...
  async function save(){
    ...
    else alert("NÃ£o foi possÃ­vel atualizar os alunos deste compromisso.");
  }
  ...
  return <div className="modal-backdrop"><section className="modal">
    <div className="modal-head">
      <div>
        <h2>Editar alunos</h2>
        <p className="muted">{event.summary} Â· marque ou desmarque os alunos desta aula.</p>
      </div>
    ...
    <div className="calendar-student-picker">
      {activeStudents.map(student=><button type="button" key={student.id} className={selected.includes(student.id)?"selected":""} onClick={()=>toggle(student.id)}>
        <span>{selected.includes(student.id)?"âœ“":"+"}</span>
    ...
    {event.recurringEventId?<div className="calendar-recurrence-scope">
      <strong>Aplicar alteraÃ§Ã£o em:</strong>
      <div className="calendar-recurrence-options">
        ...
        <button type="button" className={recurrenceScope==="following"?"selected":""} onClick={()=>setRecurrenceScope("following")}>Esta e as prÃ³ximas</button>
        <button type="button" className={recurrenceScope==="series"?"selected":""} onClick={()=>setRecurrenceScope("series")}>Toda a sÃ©rie</button>
      </div>
      {recurrenceScope==="following"?<small>As aulas anteriores permanecem como estÃ£o.</small>:null}
    </div>:null}
...
function AgendaEventDetails({event,students,compact,onOpenEvent,onOpenStudent,onStartStudent,onOpenKids,onAddStudent,onRemoveEvent}:{event:CalendarEvent;students:Student[];compact:boolean;onOpenEvent:(event:CalendarEvent)=>void;onOpenStudent:(id:string)=>void;onStartStudent:(id:string,mode:"session"|"free"|"attendance")=>void;onOpenKids:(event:CalendarEvent)=>void;onAddStudent:(event:CalendarEvent)=>void;onRemoveEvent:(id:string)=>void}){
  ...
  return <article className={`agenda-event-row agenda-event-row-multi ${compact?"agenda-list-row":""}`}><div className="agenda-event-time">{compact?<><small>{formatRailDate(event)}</small><strong>{formatCalendarTime(event)}</strong></>:formatCalendarTime(event)}</div><div className="agenda-event-body"><button className="agenda-event-title-button" onClick={()=>onOpenEvent(event)}>{event.summary}</button>{event.location?<small>ðŸ“ {event.location}</small>:null}{event.description?<small>{event.description}</small>:null}{kids?<button className="primary compact-action" onClick={()=>onOpenKids(event)}>ðŸŽ¾ Abrir turma e chamada</button>:matchedStudents.length?<div className="slot-workouts"><div className="slot-workouts-head"><span>ðŸ‹ï¸ Treinos do horÃ¡rio</span><small>{matchedStudents.length} aluno{matchedStudents.length===1?"":"s"}</small></div>{matchedStudents.map(student=>{const latestAssessment=student.assessments.slice().sort((a,b)=>b.date.localeCompare(a.date))[0];const assessmentAgeDays=latestAssessment?Math.floor((Date.now()-new Date(latestAssessment.date+"T12:00:00").getTime())/86400000):null;const assessmentExpired=assessmentAgeDays!==null&&assessmentAgeDays>60;return <div className="slot-workout-student" key={student.id}><button className="calendar-student-link" onClick={()=>onOpenStudent(student.id)}>ðŸ‘¤ {student.name}</button><span className={latestAssessment?(assessmentExpired?"home-assessment-badge expired":"home-assessment-badge ok"):"home-assessment-badge none"}>{latestAssessment?`ðŸ“ ${formatDate(latestAssessment.date)}`:"ðŸ“ Sem avaliaÃ§Ã£o"}</span><span className="slot-ficha-label">{getStudentWorkoutEntries(student).length>0?"Treino montado":"Sem treino"}</span>{getStudentWorkoutEntries(student).length>0?<button className="primary compact-action" onClick={()=>onStartStudent(student.id,"session")}>â–¶ Iniciar</button>:null}<button className={getStudentWorkoutEntries(student).length>0?"secondary compact-action":"primary compact-action"} onClick={()=>onStartStudent(student.id,"free")}>âœ Registrar</button><button className="secondary compact-action" onClick={()=>onStartStudent(student.id,"attendance")}>âœ“ PresenÃ§a</button></div>})}</div>:event.allDay?<small className="muted">Evento de dia inteiro.</small>:<small className="muted">Nenhum aluno identificado no DMP.</small>}</div><div className="agenda-event-actions"><button className="secondary compact-action" onClick={()=>onAddStudent(event)}>âœŽ Editar alunos</button>{event.htmlLink?<a className="secondary button-link compact-action" href={event.htmlLink} target="_blank" rel="noreferrer">Google</a>:null}<button className="danger-link" onClick={()=>onRemoveEvent(event.id)}>Excluir</button></div></article>;
}
...
function DataCenter({students,onReplace}:{students:Student[];onReplace:(students:Student[])=>void}) {
  ...
  async function importBackup(files:FileList|null){const file=files?.[0];if(!file)return;try{const parsed=JSON.parse(await file.text());const incoming=Array.isArray(parsed)?parsed:parsed.students;if(!Array.isArray(incoming))throw new Error();if(!confirm(`Restaurar ${incoming.length} alunos deste backup? Os dados atuais serÃ£o substituÃ­dos.`))return;onReplace(incoming);}catch{alert("Arquivo de backup invÃ¡lido.");}}
  function exportAllCsv(){const header=["Aluno","Data","Tipo","Treino","ExercÃ­cio","SÃ©ries","RepetiÃ§Ãµes","Carga","ObservaÃ§Ãµes"];const rows=students.flatMap(student=>student.sessions.flatMap(session=>session.completedExercises.length?session.completedExercises.map(ex=>[student.name,session.date,sessionSourceLabel(session),session.workoutName,ex.name,ex.sets,ex.reps,ex.load,session.notes]):[[student.name,session.date,sessionSourceLabel(session),session.workoutName,"","","","",session.notes]]));const csv=[header,...rows].map(row=>row.map(v=>`"${String(v??"").replaceAll('"','""')}"`).join(";")).join("\n");downloadText(`DMP_historico_${today()}.csv`,"\ufeff"+csv,"text/csv;charset=utf-8");}
  return <><header className="dashboard-topbar"><div><p className="dashboard-eyebrow">SeguranÃ§a e portabilidade</p><h1>Dados</h1><p>Backup, restauraÃ§Ã£o e exportaÃ§Ã£o do seu histÃ³rico.</p></div></header><section className="dashboard-content"><div className="dashboard-stats"><Stat icon="ðŸ‘¥" label="Alunos" value={students.length}/><Stat icon="âœ…" label="SessÃµes" value={sessions}/><Stat icon="ðŸ“" label="AvaliaÃ§Ãµes" value={assessments}/></div><div className="data-grid"><article className="panel"><h2>Backup completo</h2><p>Salva alunos, fichas, sessÃµes, avaliaÃ§Ãµes, observaÃ§Ãµes e restriÃ§Ãµes em um arquivo JSON.</p><button className="primary" onClick={exportBackup}>â¬‡ Baixar backup</button></article><article className="panel"><h2>Restaurar backup</h2><p>Use um arquivo gerado pelo prÃ³prio DMP. A restauraÃ§Ã£o substitui os dados locais atuais.</p><label className="secondary button-link file-button">Selecionar backup<input type="file" accept="application/json,.json" onChange={e=>importBackup(e.target.files)}/></label></article><article className="panel"><h2>Exportar histÃ³rico</h2><p>Gera um CSV Ãºnico com todas as sessÃµes de todos os alunos.</p><button className="secondary" onClick={exportAllCsv}>Exportar CSV geral</button></article><article className="panel"><h2>ImportaÃ§Ã£o de planilhas</h2><p>Estrutura reservada para a migraÃ§Ã£o das planilhas histÃ³ricas. NÃ£o altera os dados atuais atÃ© vocÃª confirmar a importaÃ§Ã£o.</p><span className="status-chip">PrÃ³xima etapa</span></article></div></section></>;
}
...
function FinancePinModal({pin,error,onChange,onClose,onSubmit}:{pin:string;error:string;onChange:(value:string)=>void;onClose:()=>void;onSubmit:(event:FormEvent)=>void}) {
  return <div className="modal-backdrop"><section className="modal"><div className="modal-head"><div><h2>Financeiro protegido</h2><p className="muted">Digite seu PIN para acessar. O Financeiro ficarÃ¡ liberado por 10 minutos.</p></div><button className="text-button" onClick={onClose}>Fechar</button></div><form className="form-grid" autoComplete="off" onSubmit={onSubmit}><label className="full">PIN<input autoFocus className="finance-pin-input" type="text" name="dmp-finance-pin" inputMode="numeric" autoComplete="one-time-code" maxLength={4} value={pin} onChange={event=>onChange(event.target.value)} placeholder="â€¢â€¢â€¢â€¢" /></label>{error?<div className="full restriction-mini">âš  {error}</div>:null}<button className="primary full" disabled={pin.length!==4}>Entrar no Financeiro</button></form></section></div>;
}
...
function Sidebar({current,onNavigate,logout}:{current:View;onNavigate:(view:View)=>void;logout:()=>void}) {
  ...
  const items:{view:View;icon:string;label:string}[]=[{view:"today",icon:"ðŸ ",label:"Hoje"},{view:"finance",icon:"ðŸ’°",label:"Financeiro"},{view:"performance",icon:"\u{1F4C8}",label:"Performance"},{view:"assessments-overview",icon:"ðŸ“",label:"AvaliaÃ§Ãµes"},{view:"students",icon:"ðŸ‘¥",label:"Alunos"},{view:"workouts-overview",icon:"ðŸ‹ï¸",label:"Treinos"},{view:"history-overview",icon:"ðŸ“‹",label:"HistÃ³rico"},{view:"agenda",icon:"ðŸ“…",label:"Agenda"},{view:"kids",icon:"ðŸŽ¾",label:"Aulas Kids"},{view:"data",icon:"ðŸ’¾",label:"Dados"},{view:"settings",icon:"âš™ï¸",label:"ConfiguraÃ§Ãµes"}];
...
function DigitalClock(){
  ...
  return <div className="header-clock" aria-label={`HorÃ¡rio atual ${hours}`}><span>â—·</span><div><small>HorÃ¡rio atual</small><strong>{hours}<em>:{seconds}</em></strong></div></div>;
}
function WeatherWidget({onOpen}:{onOpen:()=>void}){
  ...
  const icon=weather?weatherIcon(weather.code):"ðŸŒ¦ï¸";
  return <section className="header-weather" role="button" tabIndex={0} onClick={onOpen} onKeyDown={e=>{if(e.key==="Enter"||e.key===" "){e.preventDefault();onOpen();}}} title="Abrir previsÃ£o completa"><span className="header-weather-icon">{icon}</span><div>{status==="loading"?<small>Carregando clima...</small>:status==="denied"?<small>Permita a localizaÃ§Ã£o para ver o clima.</small>:status==="error"?<small>PrevisÃ£o indisponÃ­vel.</small>:weather?<><div className="header-weather-main"><b>{Math.round(weather.temperature)}Â°C</b><span>{weatherDescription(weather.code)}</span></div><div className="header-weather-meta"><span>Chuva {Math.round(weather.rainChance)}%</span><span>ðŸ’¨ {Math.round(weather.wind)} km/h</span></div></>:null}</div></section>;
}
function weatherIcon(code:number){if(code===0)return"â˜€ï¸";if(code<=3)return"â›…";if(code<=48)return"ðŸŒ«ï¸";if(code<=67)return"ðŸŒ§ï¸";if(code<=77)return"ðŸŒ¨ï¸";if(code<=82)return"ðŸŒ¦ï¸";return"â›ˆï¸";}
function weatherDescription(code:number){if(code===0)return"CÃ©u limpo";if(code<=3)return"Parcialmente nublado";if(code<=48)return"Neblina";if(code<=67)return"Chuva";if(code<=77)return"Granizo/neve";if(code<=82)return"Pancadas";return"Temporal";}
...
function WeatherPage({onBack}:{onBack:()=>void}){
  ...
  return <><header className="dashboard-topbar"><div><p className="dashboard-eyebrow">Planejamento das aulas</p><h1>PrevisÃ£o do tempo</h1><p>Clima local para os prÃ³ximos 7 dias.</p></div><button className="secondary" onClick={onBack}>â† Voltar</button></header><section className="dashboard-content weather-page">
    {status==="loading"?<div className="panel"><strong>Carregando previsÃ£o...</strong></div>:status==="denied"?<div className="panel"><strong>LocalizaÃ§Ã£o nÃ£o autorizada.</strong><p className="muted">Permita a localizaÃ§Ã£o no navegador para consultar a previsÃ£o local.</p></div>:status==="error"?<div className="panel"><strong>PrevisÃ£o indisponÃ­vel.</strong><p className="muted">O restante do DMP continua funcionando normalmente.</p></div>:<>
      {current?<section className="panel weather-current"><div><span className="weather-big-icon">{weatherIcon(current.code)}</span><div><p className="dashboard-eyebrow">Agora</p><h2>{Math.round(current.temperature)}Â°C</h2><p>{weatherDescription(current.code)} Â· Vento {Math.round(current.wind)} km/h</p></div></div></section>:null}
      <div className="weather-days">{days.map((day,i)=><article className="panel weather-day" key={day.date}><div className="weather-day-head"><span>{weatherIcon(day.code)}</span><div><strong>{i===0?"Hoje":new Date(`${day.date}T12:00:00`).toLocaleDateString("pt-BR",{weekday:"long"})}</strong><small>{new Date(`${day.date}T12:00:00`).toLocaleDateString("pt-BR",{day:"2-digit",month:"2-digit"})}</small></div></div><h3>{Math.round(day.min)}Â° / {Math.round(day.max)}Â°</h3><p>ðŸŒ§ï¸ Chuva {Math.round(day.rain)}%</p><p>ðŸ’¨ Vento atÃ© {Math.round(day.wind)} km/h</p><small>{weatherDescription(day.code)}</small></article>)}</div>
    </>}
  </section></>;
}
...
function GlobalSearch({value,onChange,students,events,onStudent,onAgenda}:{value:string;onChange:(value:string)=>void;students:Student[];events:CalendarEvent[];onStudent:(id:string)=>void;onAgenda:(date:string)=>void}){
  ...
  return <section className="global-search-box"><div className="global-search-input"><span>âŒ•</span><input value={value} onChange={event=>onChange(event.target.value)} placeholder="Busca rÃ¡pida: aluno, compromisso, telefone ou observaÃ§Ã£o..."/>{value?<button onClick={()=>onChange("")} aria-label="Limpar busca">Ã—</button>:null}</div>{query?<div className="global-search-results">{studentResults.map(student=><button key={student.id} onClick={()=>onStudent(student.id)}><span>ðŸ‘¤</span><strong>{student.name}</strong><small>Aluno Â· abrir cadastro</small></button>)}{eventResults.map(event=><button key={event.id} onClick={()=>onAgenda(calendarEventDate(event))}><span>ðŸ“…</span><strong>{event.summary}</strong><small>{formatDate(calendarEventDate(event))} Â· {formatCalendarTime(event)}</small></button>)}{!studentResults.length&&!eventResults.length?<p>Nenhum resultado encontrado.</p>:null}</div>:null}</section>;
}
...
function TodayHighlights({events,notes,students,sessions,performanceActivities,onAgenda,onStudent,onKids,onPerformance,onOpenPerformanceActivity,onNotes,onOpenNote}:{events:CalendarEvent[];notes:DmpNote[];students:Student[];sessions:{student:Student;session:Session}[];performanceActivities:PerformanceActivity[];onAgenda:(date:string)=>void;onStudent:(id:string)=>void;onKids:(event:CalendarEvent)=>void;onPerformance:()=>void;onOpenPerformanceActivity:(activity:PerformanceActivity)=>void;onNotes:()=>void;onOpenNote:(note:DmpNote)=>void}){
  ...
  const performanceIcon=(type:PerformanceActivity["type"])=>
    type==="CYCLING"?"Bike":
    type==="STRENGTH"?"ForÃ§a":
    type==="PILATES"?"Pilates":
    type==="RUNNING"?"Corrida":
    type==="TENNIS"?"TÃªnis":"Atividade";
      ...
      <button className="today-highlight-card today-notes-card" onClick={onNotes}>
        ...
        <div>
          ...
          <span className="highlight-lines">
            ...
            {pending.slice(0,4).map(note=><small key={note.id} role="button" tabIndex={0} onClick={event=>{event.stopPropagation();onOpenNote(note);}} onKeyDown={event=>{if(event.key==="Enter"||event.key===" "){event.preventDefault();event.stopPropagation();onOpenNote(note);}}}>{note.title || "Sem tÃ­tulo"}</small>)}
          </span>
        </div>
      ...
      <button className="today-highlight-card renewals-today-card" onClick={()=>{const first=renewals[0];if(first)onStudent(first.student.id);}}>
        ...
        <div>
          ...
          <span className="highlight-lines">
            ...
            {renewals.slice(0,3).map(item=><small key={item.student.id+"-"+item.workout.id}>{item.student.name} Â· Treino {item.slot} Â· {item.validity.label}</small>)}
          </span>
        </div>
      </button>
      ...
      <section className="today-summary-detail panel">
        <div className="panel-head">
          <div>
            ...
            <p className="muted">Resumo dos alunos programados, jÃ¡ atendidos, pendentes e ausentes.</p>
          </div>
        ...
        <div className="today-summary-columns">
          {renderPeople("Atendidos",attended,"Nenhum atendimento concluÃ­do ainda.","done")}
          {renderPeople("Ainda faltam",remaining,"Nenhum atendimento pendente.","pending")}
          {renderPeople("Ausentes",absent,"Nenhuma ausÃªncia registrada.","absent")}
        </div>
      </section>
    :null}
  </>;
...
function MiniMonthCalendar({onSelect}:{onSelect:(date:string)=>void}){
  ...
  return <article className="mini-month"><div className="mini-month-nav"><button onClick={()=>setCursor(new Date(year,month-1,1))}>â€¹</button><strong>{cursor.toLocaleDateString("pt-BR",{month:"long",year:"numeric"})}</strong><button onClick={()=>setCursor(new Date(year,month+1,1))}>â€º</button></div><button className="mini-month-today" onClick={()=>{setCursor(new Date(now.getFullYear(),now.getMonth(),1));onSelect(today());}}>Hoje Â· abrir agenda</button><div className="mini-month-week"><b>D</b><b>S</b><b>T</b><b>Q</b><b>Q</b><b>S</b><b>S</b></div><div className="mini-month-days">{Array.from({length:first},(_,index)=><i key={`e-${index}`}/>)}{Array.from({length:days},(_,index)=>{const day=index+1;const value=`${year}-${String(month+1).padStart(2,"0")}-${String(day).padStart(2,"0")}`;return <button key={day} className={value===today()?"today":""} onClick={()=>onSelect(value)}>{day}</button>;})}</div></article>;
}
  ...
  return <>
    <aside className="desktop-agenda-rail">
      ...
      <div className="agenda-rail-scroll">
        ...
        }):<p>Nenhum prÃ³ximo compromisso.</p>}
      </div>
    </aside>
...
function MobileQuickActions({onClose,onNavigate}:{onClose:()=>void;onNavigate:(target:"extra"|"receive"|"students"|"kids")=>void}){
  return <div className="mobile-actions-backdrop" onClick={onClose}><section className="mobile-actions-sheet" onClick={event=>event.stopPropagation()}><div><strong>AÃ§Ãµes rÃ¡pidas</strong><button onClick={onClose}>Ã—</button></div><button className="primary" onClick={()=>onNavigate("extra")}>ðŸ’¸ LanÃ§ar gasto extra</button><button onClick={()=>onNavigate("receive")}>ðŸ’° Registrar recebimento</button><button onClick={()=>onNavigate("students")}>âœ Registrar treino ou presenÃ§a</button><button onClick={()=>onNavigate("kids")}>ðŸŽ¾ Abrir Aulas Kids</button></section></div>;
}
...
function Header({title,back}:{title:string;back?:()=>void}) { return <header className="topbar"><div className="header-left">{back ? <button className="text-button" onClick={back}>â† Voltar</button> : null}<img src="/logo-danilo.jpg" alt="Danilo Modesto" className="header-logo" /><strong>{title}</strong></div></header>; }
...
function StudentProfileSnapshot({student}:{student:Student}) {
  ...
  const slotLabel=workouts.length?workouts.map(entry=>entry.slot).join(" Â· "):"Sem ficha";
  return <section className="profile-snapshot"><div className="snapshot-card"><span>Idade</span><strong>{age!==null?`${age} anos`:"â€”"}</strong></div><div className="snapshot-card"><span>Modalidade</span><strong>{student.modality||"â€”"}</strong></div><div className="snapshot-card"><span>Treinos montados</span><strong>{slotLabel}</strong></div><div className="snapshot-card"><span>Ãšltima sessÃ£o</span><strong>{lastSession?formatDate(lastSession.date):"â€”"}</strong></div>{alerts.length?<div className="snapshot-alert"><span>âš  Lembretes importantes</span><strong>{alerts.join(" Â· ")}</strong></div>:<div className="snapshot-alert clear"><span>âœ“ Cuidados</span><strong>Nenhuma restriÃ§Ã£o ou dor registrada</strong></div>}</section>;
}
...
async function printPersonalStudentReport(student:Student){
  ...
  if(!popup){alert("Permita a abertura de janelas para gerar o relatÃ³rio.");return;}
  popup.document.write("<p style='font-family:Arial;padding:24px'>Preparando relatÃ³rio...</p>");
  ...
  const html=`<!doctype html><html><head><meta charset="utf-8"><title>RelatÃ³rio Â· ${esc(student.name)}</title><style>@page{size:A4;margin:14mm}*{box-sizing:border-box}body{font-family:Arial;color:#13202b;margin:0}header{display:flex;justify-content:space-between;align-items:center;border-bottom:4px solid #abd92f;padding-bottom:12px}header img{width:170px}h1{margin:0;font-size:25px}h2{font-size:17px;margin:22px 0 8px;color:#126d94}.muted{color:#65717b}.cards{display:grid;grid-template-columns:repeat(4,1fr);gap:8px;margin:18px 0}.card{border:1px solid #dfe5e8;border-radius:10px;padding:10px}.card small{display:block;color:#65717b}.card strong{font-size:18px}table{width:100%;border-collapse:collapse;font-size:12px}th,td{text-align:left;padding:7px;border-bottom:1px solid #e3e7e9;vertical-align:top}th{background:#f2f7e9}footer{margin-top:28px;border-top:1px solid #ddd;padding-top:8px;font-size:11px;color:#777}@media print{button{display:none}}</style></head><body><header><div><p class="muted">RELATÃ“RIO INDIVIDUAL</p><h1>${esc(student.name)}</h1><p>${esc(student.goal||"Objetivo nÃ£o informado")}</p></div><img src="/logo-danilo.jpg"></header><div class="cards"><div class="card"><small>SessÃµes</small><strong>${student.sessions.length}</strong></div><div class="card"><small>AvaliaÃ§Ãµes</small><strong>${student.assessments.length}</strong></div><div class="card"><small>Recebido</small><strong>${money(paid)}</strong></div><div class="card"><small>Em aberto</small><strong>${money(Math.max(0,expected-paid))}</strong></div></div><h2>FrequÃªncia por mÃªs</h2><table><thead><tr><th>MÃªs</th><th>Registros</th><th>Fichas</th><th>Treinos livres</th><th>PresenÃ§as</th></tr></thead><tbody>${[...months.entries()].sort((a,b)=>b[0].localeCompare(a[0])).map(([key,list])=>`<tr><td>${monthLabel(key)}</td><td>${list.length}</td><td>${list.filter(item=>(item.source||"PLANNED")==="PLANNED").length}</td><td>${list.filter(item=>item.source==="FREE").length}</td><td>${list.filter(item=>item.source==="ATTENDANCE").length}</td></tr>`).join("")||"<tr><td colspan='5'>Nenhum registro.</td></tr>"}</tbody></table><h2>Treinos e evoluÃ§Ã£o recente</h2><table><thead><tr><th>Data</th><th>Treino</th><th>ExercÃ­cios / cargas</th></tr></thead><tbody>${student.sessions.slice().sort((a,b)=>b.date.localeCompare(a.date)).slice(0,30).map(session=>`<tr><td>${formatDate(session.date)}</td><td>${esc(session.workoutName)}</td><td>${session.completedExercises.map(exercise=>`${esc(exercise.name)}${exercise.sets||exercise.reps?` Â· ${esc(exercise.sets)}Ã—${esc(exercise.reps)}`:""}${exercise.load?` Â· ${esc(exercise.load)}`:""}`).join("<br>")||"PresenÃ§a sem detalhamento"}</td></tr>`).join("")||"<tr><td colspan='3'>Nenhuma sessÃ£o.</td></tr>"}</tbody></table><h2>Financeiro</h2><table><thead><tr><th>CompetÃªncia</th><th>Vencimento</th><th>Previsto</th><th>Pago</th><th>Saldo</th></tr></thead><tbody>${invoices.map(item=>{const received=item.payments.reduce((sum,payment)=>sum+payment.amount,0);return `<tr><td>${esc(item.competence)}</td><td>Dia ${item.dueDay}</td><td>${money(item.expectedAmount)}</td><td>${money(received)}</td><td>${money(Math.max(0,item.expectedAmount-received))}</td></tr>`}).join("")||"<tr><td colspan='5'>Nenhum lanÃ§amento financeiro localizado.</td></tr>"}</tbody></table><footer>Danilo Modesto Personal Trainer Â· Emitido em ${new Date().toLocaleString("pt-BR")}</footer><script>window.onload=()=>setTimeout(()=>window.print(),300)<\/script></body></html>`;
...
function StudentSummary({student}:{student:Student}) {
  ...
  return <div className="detail-grid"><article className="panel"><h2>Resumo rÃ¡pido</h2><dl className="summary-list"><div><dt>Aluno desde</dt><dd>{student.startDate ? formatDate(student.startDate) : "NÃ£o informado"}</dd></div><div><dt>Tempo com vocÃª</dt><dd>{months === null ? "NÃ£o informado" : formatMonths(months)}</dd></div><div><dt>Nascimento</dt><dd>{student.birthDate ? `${formatDate(student.birthDate)}${age !== null ? ` (${age} anos)` : ""}` : "NÃ£o informado"}</dd></div><div><dt>Telefone</dt><dd>{student.phone || "NÃ£o informado"}</dd></div><div><dt>E-mail</dt><dd>{student.email || "NÃ£o informado"}</dd></div><div><dt>ProfissÃ£o</dt><dd>{student.profession || "NÃ£o informado"}</dd></div><div><dt>Modalidade</dt><dd>{student.modality || "NÃ£o informado"}</dd></div><div><dt>FrequÃªncia</dt><dd>{student.weeklyFrequency || "NÃ£o informado"}</dd></div><div><dt>Treinos montados</dt><dd>{slotLabel}</dd></div></dl></article><article className="panel safety-panel"><h2>âš  Cuidados do aluno</h2><div className="safety-block important"><strong>RestriÃ§Ãµes / cuidados</strong><p>{student.restrictions || "Nenhuma restriÃ§Ã£o registrada."}</p></div><div className="safety-block"><strong>LesÃµes / dores</strong><p>{student.injuries || "Nenhuma lesÃ£o ou dor registrada."}</p></div><div className="safety-block"><strong>MedicaÃ§Ãµes / informaÃ§Ãµes relevantes</strong><p>{student.medications || "Nenhuma informaÃ§Ã£o registrada."}</p></div><div className="safety-block"><strong>ObservaÃ§Ãµes gerais</strong><p>{student.notes || "Nenhuma observaÃ§Ã£o registrada."}</p></div>{student.emergencyContact||student.emergencyPhone?<div className="safety-block"><strong>Contato de emergÃªncia</strong><p>{[student.emergencyContact,student.emergencyPhone].filter(Boolean).join(" Â· ")}</p></div>:null}</article></div>;
}
...
function HomeClosingSummary({students,performanceActivities}:{students:Student[];performanceActivities:PerformanceActivity[]}){
  ...
  return <section className="home-closing-section">
    <div className="panel-head">
      <div>
        <h2>Fechamentos</h2>
        <p className="muted">Uma leitura rÃ¡pida do trabalho realizado, sem dados financeiros.</p>
      </div>
    </div>
    ...
    <div className="home-closing-grid">
      <article className="home-closing-card">
        <div className="home-closing-title">
          <span>âœ“</span>
          <div>
            <small>FECHAMENTO DO ÃšLTIMO DIA</small>
        ...
        {lastDate?<div className="home-closing-metrics">
          ...
          <div><span>AusÃªncias</span><b>{lastAbsent}</b></div>
          <div><span>Treinos detalhados</span><b>{lastDetailed}</b></div>
          <div><span>AvaliaÃ§Ãµes</span><b>{lastAssessments.length}</b></div>
          <div><span>Seu treino</span><b>{lastPerformance.length}</b></div>
          <div><span>Registros totais</span><b>{lastSessions.length}</b></div>
        </div>:<p className="muted">Ainda nÃ£o existe atividade anterior para resumir.</p>}
      </article>
      ...
      <article className="home-closing-card month">
        <div className="home-closing-title">
          <span>â˜°</span>
          <div>
            <small>FECHAMENTO DO MÃŠS</small>
        ...
        <div className="home-closing-metrics">
          ...
          <div><span>AusÃªncias</span><b>{monthAbsent}</b></div>
          <div><span>Treinos detalhados</span><b>{monthDetailed}</b></div>
          <div><span>AvaliaÃ§Ãµes</span><b>{monthAssessments.length}</b></div>
          <div><span>Seus treinos</span><b>{monthPerformance.length}</b></div>
          <div><span>DistÃ¢ncia pessoal</span><b>{monthDistance?monthDistance.toLocaleString("pt-BR",{maximumFractionDigits:1})+" km":"â€”"}</b></div>
        </div>
      </article>
    </div>
  </section>;
    ...
    <section className="panel intelligent-history-panel">
      ...
      <div className="intelligent-history-details">
        <div>
          ...
          <strong>{latestSession?formatDate(latestSession.date):"â€”"}</strong>
          <small>{latestSession?.workoutName||"Nenhum registro"}</small>
        </div>

        <div>
          <span>{"\u00DAltima avalia\u00E7\u00E3o"}</span>
          <strong>{latestAssessment?formatDate(latestAssessment.date):"â€”"}</strong>
...
function assessmentMetric(value:number|null|undefined,suffix=""){return value===null||value===undefined||Number.isNaN(value)?"â€”":`${value.toLocaleString("pt-BR",{maximumFractionDigits:1})}${suffix}`;}
...
function openAssessmentReport(student:Student,assessment:Assessment){
  ...
  const measurements=Object.entries(assessment.measurements||{}).filter(([,value])=>String(value||"").trim()).map(([key,value])=>`<tr><td>${esc(MEASUREMENT_LABELS[key]||key)}</td><td>${esc(value)} cm</td>${previous?.measurements?.[key as keyof Measurements]?`<td>${esc(previous.measurements[key as keyof Measurements])} cm</td>`:"<td>â€”</td>"}</tr>`).join("");
...
function AssessmentPanel({student,onNew}:{student:Student;onNew:()=>void}) {
  ...
  return <section className="panel assessment-history-panel">
    <div className="panel-head">
      <div>
        <h2>AvaliaÃ§Ãµes</h2>
        <p className="muted">HistÃ³rico corporal e evoluÃ§Ã£o do aluno.</p>
      </div>
      <button className="primary" onClick={onNew}>+ Nova avaliaÃ§Ã£o</button>
    </div>
    ...
    {latest?
      <section className="assessment-comparison">
        <div className="assessment-comparison-head">
          <div>
            <span>COMPARAÃ‡ÃƒO MAIS RECENTE</span>
            <strong>{formatDate(latest.date)}</strong>
          </div>
          <small>{previous?`Comparado com ${formatDate(previous.date)}`:"Primeira avaliaÃ§Ã£o registrada"}</small>
        </div>
        ...
        <div className="assessment-compare-grid">
          {comparison.map(item=>
            <article key={item.label}>
              ...
              <small>{item.delta?`${item.delta} vs. anterior`:"Sem comparaÃ§Ã£o anterior"}</small>
            </article>
          )}
        </div>
      </section>
          ...
          return <article className="assessment-card assessment-card-rich" key={assessment.id}>
            <div className="assessment-card-main">
              ...
              <div className="assessment-card-actions">
                <button className="primary" onClick={()=>openAssessmentReport(student,assessment)}>Ver relatÃ³rio</button>
                {previousAssessment?<small>ComparaÃ§Ã£o com {formatDate(previousAssessment.date)}</small>:<small>Primeira avaliaÃ§Ã£o</small>}
              </div>
            </div>
            ...
            {assessment.photos.length?
              <div className="assessment-photos">
                {assessment.photos.slice(0,4).map((photo,photoIndex)=>
                  <img key={photoIndex} src={photo} alt={`AvaliaÃ§Ã£o ${photoIndex+1}`}/>
                )}
              </div>
            :null}
          </article>
    ...
    :<p className="muted">Nenhuma avaliaÃ§Ã£o registrada.</p>}
  </section>;
}
...
function studentFileCategoryLabel(category:StudentFileCategory){
  if(category==="MEDICAL_CERTIFICATE")return "Atestado mÃ©dico";
  if(category==="EXAM")return "Exame";
  if(category==="ASSESSMENT")return "AvaliaÃ§Ã£o";
...
function StudentFilesPanel({student}:{student:Student}) {
  ...
  async function saveFile(){
    ...
    if(selectedFile.size>5*1024*1024){
      alert("O arquivo deve ter no mÃ¡ximo 5 MB.");
      return;
    }
    ...
    try{
      ...
      console.error(error);
      alert("NÃ£o foi possÃ­vel salvar o arquivo.");
    }finally{
      setSaving(false);
    }
  }
  ...
  async function removeFile(file:StudentFileRecord){
    if(!confirm(`Excluir â€œ${file.name}â€?`))return;
    ...
    try{
      ...
      alert("NÃ£o foi possÃ­vel excluir o arquivo.");
    }
  }
  ...
  return <section className="panel student-files-panel">
    <div className="panel-head">
      <div>
        <h2>ðŸ“ Arquivos do aluno</h2>
        <p className="muted">Atestados, exames, avaliaÃ§Ãµes, laudos e outros documentos vinculados a {student.name}.</p>
      </div>
    </div>
    ...
    <div className="student-file-upload">
      ...
      <label>
        ...
        >
          <option value="MEDICAL_CERTIFICATE">Atestado mÃ©dico</option>
          <option value="EXAM">Exame</option>
          <option value="ASSESSMENT">AvaliaÃ§Ã£o</option>
      ...
      <label className="student-file-notes">
        ...
        <input
          ...
          placeholder="Ex.: retorno mÃ©dico, exame de joelho..."
        />
      </label>
    ...
    :
      <div className="empty-review">
        ...
        <span>Use esta Ã¡rea para organizar documentos importantes deste aluno.</span>
      </div>
    }
  </section>;
}
...
function StudentForm({title,initialStudent,onClose,onSave}:{title:string;initialStudent?:Student;onClose:()=>void;onSave:(payload:StudentFormPayload)=>void}) {
  ...
  return <div className="modal-backdrop"><section className="modal modal-large"><div className="modal-head"><div><h2>{title}</h2><p className="muted">Cadastro completo para atendimento e seguranÃ§a.</p></div><button className="text-button" onClick={onClose}>Fechar</button></div><form className="form-grid" onSubmit={submit}>
    ...
    <label>E-mail<input type="email" value={form.email||""} onChange={e=>setForm({...form,email:e.target.value})} /></label><label>ProfissÃ£o<input value={form.profession||""} onChange={e=>setForm({...form,profession:e.target.value})} /></label>
    <label>Data de inÃ­cio<input type="date" value={form.startDate} onChange={e=>setForm({...form,startDate:e.target.value})} /></label><label>Data de nascimento<input type="date" value={form.birthDate} onChange={e=>setForm({...form,birthDate:e.target.value})} />{age!==null?<small>Idade atual: {age} anos</small>:null}</label><label>Sexo<select value={form.gender||""} onChange={e=>setForm({...form,gender:(e.target.value||undefined) as Student["gender"]})}><option value="">NÃ£o informado</option><option value="MALE">Masculino</option><option value="FEMALE">Feminino</option></select></label>
    <label>Modalidade<input value={form.modality||""} onChange={e=>setForm({...form,modality:e.target.value})} placeholder="Ex.: musculaÃ§Ã£o, tÃªnis, corrida" /></label><label>FrequÃªncia semanal<input value={form.weeklyFrequency||""} onChange={e=>setForm({...form,weeklyFrequency:e.target.value})} placeholder="Ex.: 2x por semana" /></label>
    <fieldset className="full tennis-category-picker"><legend>Categoria DS TÃªnis</legend>{([null,"RED","ORANGE","GREEN"] as const).map(category=><button type="button" key={category||"NONE"} className={form.tennisCategory===category?"selected":""} onClick={()=>setForm({...form,tennisCategory:category})}><StudentCategoryDot category={category}/>{category===null?"Sem categoria":category==="RED"?"Vermelha":category==="ORANGE"?"Laranja":"Verde"}</button>)}</fieldset>
    <label className="full">Objetivo<input value={form.goal} onChange={e=>setForm({...form,goal:e.target.value})} /></label>
    <label className="full">âš  RestriÃ§Ãµes / cuidados importantes<textarea rows={3} value={form.restrictions} onChange={e=>setForm({...form,restrictions:e.target.value})} placeholder="O que vocÃª precisa lembrar antes de prescrever ou iniciar a aula." /></label>
    <label className="full">LesÃµes / dores<textarea rows={3} value={form.injuries||""} onChange={e=>setForm({...form,injuries:e.target.value})} /></label>
    <label className="full">MedicaÃ§Ãµes / informaÃ§Ãµes relevantes<textarea rows={2} value={form.medications||""} onChange={e=>setForm({...form,medications:e.target.value})} /></label>
    <label>Contato de emergÃªncia<input value={form.emergencyContact||""} onChange={e=>setForm({...form,emergencyContact:e.target.value})} /></label><label>Telefone emergÃªncia<input value={form.emergencyPhone||""} onChange={e=>setForm({...form,emergencyPhone:e.target.value})} /></label>
    <label className="full">ObservaÃ§Ãµes gerais<textarea rows={4} value={form.notes} onChange={e=>setForm({...form,notes:e.target.value})} /></label><button className="primary full">Salvar aluno</button></form></section></div>;
}
...
function DuplicateWorkoutModal({workout,students,sourceStudentId,onClose,onConfirm}:{workout:Workout;students:Student[];sourceStudentId:string;onClose:()=>void;onConfirm:(studentId:string,slot:WorkoutSlot)=>void}){
  ...
  return <div className="modal-backdrop"><section className="modal"><div className="modal-head"><div><h2>Duplicar ficha</h2><p className="muted">Copie â€œ{workout.name}â€ para outro aluno. A cÃ³pia poderÃ¡ ser editada normalmente.</p></div><button className="text-button" onClick={onClose}>Fechar</button></div><div className="form-grid"><label className="full">Aluno<select value={studentId} onChange={event=>setStudentId(event.target.value)}>{targets.map(student=><option key={student.id} value={student.id}>{student.name}</option>)}</select></label><label className="full">Destino<select value={slot} onChange={event=>setSlot(event.target.value as WorkoutSlot)}>{WORKOUT_SLOTS.map(value=><option key={value} value={value}>Treino {value}</option>)}</select></label><button className="primary full" disabled={!studentId} onClick={()=>onConfirm(studentId,slot)}>Duplicar ficha</button></div></section></div>;
}
  ...
  return <div className="workout-section-stack">
    <section className="panel workout-library-panel">
      <div className="panel-head workout-library-head">
        <div><h2>Treinos planejados</h2><p className="muted">Cada aba Ã© independente. Monte A, B, C e D com o protocolo que quiser.</p></div>
      ...
      <div className="workout-tabs-grid">{WORKOUT_SLOTS.map(slot=>{
        ...
        return <article className={`workout-slot-card ${workout?"filled":"empty"}`} key={slot}>
          ...
          {workout?<><h3>{workout.name||`Treino ${slot}`}</h3><p className="workout-slot-meta">{workout.exercises.length} exercÃ­cio{workout.exercises.length===1?"":"s"} Â· {workout.sequenceSize||defaultSequenceSize(workout.protocol||"CONVENTIONAL")} por sequÃªncia</p>{validity&&validity.status!=="NONE"?<div className={`workout-validity-badge ${validity.status.toLowerCase()}`}><strong>{validity.label}</strong>{workout.validityMode==="PERIOD"&&validity.endDate?<small>Termina em {formatDate(validity.endDate)}</small>:workout.validityMode==="SESSIONS"?<small>{validity.completed}/{workout.validitySessionTarget||0} sessÃµes realizadas</small>:null}</div>:null}{workout.notes?<p className="workout-slot-note">{workout.notes}</p>:null}<ul className="workout-slot-preview">{workout.exercises.slice(0,6).map((exercise,index)=><li key={exercise.id}><strong>{index+1}.</strong> {exercise.name}<small>{exercise.sets||exercise.reps?`${exercise.sets}Ã—${exercise.reps}`:""}{exercise.load?` Â· ${exercise.load}`:""}</small></li>)}</ul>{workout.exercises.length>6?<small className="muted">+ {workout.exercises.length-6} exercÃ­cios</small>:null}<div className="workout-slot-actions workout-slot-actions-share"><button className="secondary" onClick={()=>onEdit(slot,workout)}>Editar</button><button className="secondary" onClick={()=>onCopy(workout)}>Duplicar</button><button className="secondary" onClick={()=>openWorkoutSharePreview(student,workout)}>Compartilhar</button><button className="secondary archive-workout-button" onClick={()=>onArchive(workout)}>Arquivar</button><button className="secondary clear-workout-button" onClick={()=>onClear(workout)}>ðŸ—‘ Limpar treino</button><button className="primary" onClick={()=>onStart(workout)}>â–¶ Iniciar treino</button></div></>:<><div className="workout-slot-empty"><strong>Treino {slot} ainda nÃ£o montado</strong><span>Escolha o protocolo e adicione os exercÃ­cios.</span></div><button className="primary" onClick={()=>onEdit(slot,null)}>+ Montar Treino {slot}</button></>}
        </article>
      })}</div>
    </section>
    ...
    <section className="panel archived-workouts-panel">
      <div className="panel-head workout-library-head">
        <div><h2>Treinos arquivados</h2><p className="muted">Fichas antigas ficam guardadas aqui para consulta. Arquivar nÃ£o altera o histÃ³rico das aulas.</p></div>
      ...
      {archived.length?<div className="archived-workout-list">{archived.map(workout=>{
        ...
        const archivedName=slot?`Treino ${slot} Â· ${workout.name||`Treino ${slot}`}`:workout.name||"Ficha anterior";
        return <details className="archived-workout-item" key={workout.id}>
          <summary>
            <span><strong>{archivedName}</strong><small>{workoutProtocolLabel(workout.protocol)} Â· {workout.exercises.length} exercÃ­cio{workout.exercises.length===1?"":"s"}</small></span>
          ...
          <ul className="simple-list archived-exercise-list">{workout.exercises.map((exercise,index)=><li key={exercise.id}><strong>{index+1}. {exercise.block?`${exercise.block} Â· `:""}{exercise.name}</strong>{exercise.sets||exercise.reps?` â€” ${exercise.sets}Ã—${exercise.reps}`:""}{exercise.load?` â€” ${exercise.load}`:""}{exercise.notes?` â€” ${exercise.notes}`:""}</li>)}</ul>
        </details>
      })}</div>:<div className="empty-review"><strong>Nenhum treino arquivado</strong><span>Quando vocÃª trocar uma ficha, use â€œArquivarâ€ para guardar o planejamento antigo e liberar a aba.</span></div>}
    </section>
  </div>;
}
...
function WorkoutEditor({student,workout,slot,exerciseCatalog,personalTemplates,onTemplatesChange,onBack,onSave}:{student:Student;workout:Workout|null;slot:WorkoutSlot;exerciseCatalog:string[];personalTemplates:PersonalWorkoutTemplate[];onTemplatesChange:(templates:PersonalWorkoutTemplate[])=>void;onBack:()=>void;onSave:(workout:Workout)=>void}) {
  ...
  function listenWorkout(){
    ...
    if(!Recognition){alert("Ditado por voz nÃ£o disponÃ­vel neste navegador. VocÃª pode colar ou digitar o treino.");return;}
  ...
  function applyPersonalTemplate(template:PersonalWorkoutTemplate){
    if(exercises.some(exercise=>exercise.name.trim())&&!confirm(`Substituir os exercÃ­cios atuais pelo modelo â€œ${template.name}â€?`))return;
  ...
  function saveAsPersonalTemplate(){
    ...
    if(!cleanExercises.length){
      alert("Adicione pelo menos um exercÃ­cio antes de salvar como modelo.");
      return;
    }
    ...
    if(personalTemplates.some(item=>normalizeName(item.name)===normalizeName(templateName))){
      alert("JÃ¡ existe um modelo com esse nome. Escolha outro nome para evitar duplicidade na biblioteca.");
      return;
    }

    const description=prompt(
      "DescriÃ§Ã£o curta do modelo:",
      `${workoutProtocolLabel(protocol)} Â· ${cleanExercises.length} exercÃ­cios`
    )?.trim()||"";
    ...
    alert(`Modelo â€œ${templateName}â€ salvo na Biblioteca de Treinos.`);
  }
  ...
  function renamePersonalTemplate(template:PersonalWorkoutTemplate){
    ...
    if(personalTemplates.some(item=>item.id!==template.id&&normalizeName(item.name)===normalizeName(nextName))){
      alert("JÃ¡ existe outro modelo com esse nome.");
      return;
    }
  ...
  function deletePersonalTemplate(template:PersonalWorkoutTemplate){
    if(!confirm(`Excluir o modelo â€œ${template.name}â€ da biblioteca?`))return;
    onTemplatesChange(personalTemplates.filter(item=>item.id!==template.id));
  }

  function applyTemplate(template:WorkoutTemplate){
    if(exercises.some(exercise=>exercise.name.trim())&&!confirm(`Substituir os exercÃ­cios atuais pelo modelo â€œ${template.label}â€?`))return;
    setProtocol(template.protocol);
    setSequenceSize(template.sequenceSize);
    setWorkoutNotes(template.notes||"");
  ...
  return <main className="app-page"><Header title={`${student.name} â€” Treino ${slot}`} back={onBack}/><section className="content workout-editor-page">
    {student.restrictions||student.injuries?<div className="session-alert"><strong>âš  AtenÃ§Ã£o com {student.name}</strong><span>{[student.restrictions,student.injuries].filter(Boolean).join(" Â· ")}</span></div>:null}
    <section className="panel workout-config-panel">
      ...
      <div className="workout-editor-title">
        <div>
          ...
          <p>Monte o treino e defina por quanto tempo esta ficha ficarÃ¡ ativa para o aluno.</p>
        </div>
      ...
      <section className="workout-validity-main">
        <div className="workout-validity-main-head">
          <div>
            <span className="workout-validity-kicker">VIGÃŠNCIA DO TREINO</span>
            <h2>Por quanto tempo este treino serÃ¡ utilizado?</h2>
            <p>Defina agora para o DMP avisar quando estiver perto da renovaÃ§Ã£o.</p>
          </div>
        </div>
        ...
        <div className="workout-validity-options">
          ...
          >
            ...
            <small>O treino permanece ativo atÃ© vocÃª trocar ou arquivar.</small>
          </button>
          ...
          >
            ...
            <small>O DMP conta as sessÃµes realizadas atÃ© a renovaÃ§Ã£o.</small>
          </button>
        </div>
            ...
            <div className="workout-validity-preview">
              ...
              {validityMode==="PERIOD"?
                <>
                  ...
                  <small>
                    {validityEndPreview
                      ?`Treino vÃ¡lido atÃ© ${formatDate(validityEndPreview)}`
                      :"Informe a data de inÃ­cio"}
                  </small>
                </>
              :
                <>
                  <strong>{validitySessionTarget} treino{validitySessionTarget===1?"":"s"}</strong>
                  <small>O DMP avisarÃ¡ quando restarem somente 2 sessÃµes.</small>
                </>
              }
            </div>
          </div>
        :
          <div className="workout-validity-no-control">
            <strong>Sem prazo de renovaÃ§Ã£o definido</strong>
            <span>VocÃª poderÃ¡ alterar a vigÃªncia deste treino depois.</span>
          </div>
        }
      </section>
      ...
      <label>
        ObservaÃ§Ãµes da ficha
        <textarea
          rows={3}
          value={workoutNotes}
          onChange={e=>setWorkoutNotes(e.target.value)}
          placeholder="Ex.: atenÃ§Ã£o ao intervalo, progressÃ£o planejada, ordem especial..."
        />
      </label>
    ...
    <section className="panel workout-template-panel">
      <div className="panel-head">
        <div>
          <h2>ðŸ§© Biblioteca de Treinos</h2>
      ...
      <div className="personal-workout-library">
            ...
            {personalTemplates.map(template=>
              <article className="personal-workout-template-card" key={template.id}>
                <div>
                  ...
                  <h3>{template.name}</h3>
                  <p>{template.description||`${template.exercises.length} exercÃ­cios`}</p>
                  <small>{template.exercises.length} exercÃ­cio{template.exercises.length===1?"":"s"}</small>
                </div>
        ...
        :
          <div className="empty-review compact-empty">
            ...
            <span>Monte uma ficha e toque em â€œSalvar como modeloâ€.</span>
          </div>
        }
      </div>

      <div className="workout-template-divider">
        <strong>Modelos rÃ¡pidos do DMP</strong>
        <span>Estruturas prontas para comeÃ§ar do zero.</span>
      </div>
    ...
    <section className="panel workout-dictation-panel"><div className="panel-head"><div><h2>ðŸ“‹ Importar treino por texto</h2><p className="muted">Cole aqui o treino organizado e transforme em ficha para revisÃ£o. Nada Ã© salvo automaticamente.</p></div></div><textarea rows={8} value={dictation} onChange={e=>setDictation(e.target.value)} placeholder={'Treino em sistema B7\n\nBloco 1\nSupino reto â€” 3x15 â€” 30 kg\nAgachamento livre â€” 3x15\n\nBloco 2\nSupino inclinado â€” 3x12 â€” 12 kg de cada lado\nCadeira extensora â€” 3x15'}/><div className="hero-actions"><button className="primary" onClick={applyDictation} disabled={!dictation.trim()}>Interpretar texto</button><button className="secondary" onClick={listenWorkout}>{dictating?"Ouvindo...":"ðŸŽ¤ Falar (experimental)"}</button></div><small className="muted">Depois de interpretar, confira protocolo, blocos, sÃ©ries, repetiÃ§Ãµes e cargas na tabela abaixo. O microfone continua disponÃ­vel, mas Ã© experimental.</small></section>

    <section className="panel workout-grid-panel"><div className="panel-head"><div><h2>ExercÃ­cios do Treino {slot}</h2><p className="muted">Comece a digitar um exercÃ­cio jÃ¡ usado para ver sugestÃµes.</p></div><button className="primary" onClick={addExercise}>+ ExercÃ­cio</button></div><datalist id="dmp-exercise-catalog">{exerciseCatalog.map(name=><option key={name} value={name}/>)}</datalist>{exercises.length?<div className="workout-table"><div className="workout-table-head"><span>#</span><span>Seq.</span><span>ExercÃ­cio</span><span>SÃ©ries</span><span>Reps</span><span>Carga</span><span>ObservaÃ§Ã£o</span><span></span></div>{exercises.map((exercise,index)=><div className="workout-table-row" key={exercise.id}><strong>{index+1}</strong><input aria-label="SequÃªncia" placeholder={sequenceBlockLabel(protocol,index,sequenceSize)||"â€”"} value={exercise.block||""} onChange={e=>updateExercise(exercise.id,{block:e.target.value})}/><input className="workout-exercise-name" list="dmp-exercise-catalog" placeholder="ExercÃ­cio" value={exercise.name} onChange={e=>updateExercise(exercise.id,{name:e.target.value})}/><input placeholder="SÃ©ries" value={exercise.sets} onChange={e=>updateExercise(exercise.id,{sets:e.target.value})}/><input placeholder="Reps" value={exercise.reps} onChange={e=>updateExercise(exercise.id,{reps:e.target.value})}/><input placeholder="Carga" value={exercise.load} onChange={e=>updateExercise(exercise.id,{load:e.target.value})}/><input placeholder="ObservaÃ§Ã£o" value={exercise.notes||""} onChange={e=>updateExercise(exercise.id,{notes:e.target.value})}/><button className="danger-link workout-remove" onClick={()=>removeExercise(exercise.id)}>Ã—</button></div>)}</div>:<div className="empty-review"><strong>Nenhum exercÃ­cio ainda</strong><span>Toque em â€œ+ ExercÃ­cioâ€ para comeÃ§ar a montar o Treino {slot}.</span></div>}{removedExercise?<div className="undo-strip"><span>ExercÃ­cio removido.</span><button onClick={undoExerciseRemoval}>Desfazer</button></div>:null}<div className="workout-editor-footer"><button className="secondary" onClick={addExercise}>+ Adicionar exercÃ­cio</button><button className="primary" disabled={!exercises.some(ex=>ex.name.trim())} onClick={save}>Salvar Treino {slot}</button></div></section>
  </section></main>;
}
...
function PlannedSession({student,workout,onBack,onSave}:{student:Student;workout:Workout|null;onBack:()=>void;onSave:(session:Session)=>void}) {
  ...
  if(lessonMode && currentExercise){
    ...
    return <main className="app-page lesson-mode-page"><Header title={`${student.name} â€” Treino ${slot}`} back={()=>setLessonMode(false)}/><section className="content lesson-mode-content"><div className="planned-student-identity"><span>ALUNO</span><strong>{student.name}</strong><small>Treino {slot}</small></div>
      {student.restrictions||student.injuries?<div className="session-alert"><strong>âš  AtenÃ§Ã£o com {student.name}</strong><span>{[student.restrictions,student.injuries].filter(Boolean).join(" Â· ")}</span></div>:null}
      <div className="lesson-progress"><span>{workoutProtocolLabel(protocol)} Â· ExercÃ­cio {currentIndex+1} de {exercises.length}</span><div><i style={{width:`${((currentIndex+1)/Math.max(1,exercises.length))*100}%`}}/></div></div>
      <article className="panel lesson-card"><div className="lesson-card-top"><span className="status-chip">{currentExercise.block||`#${currentIndex+1}`}</span><label className="exercise-check"><input type="checkbox" checked={completed[currentExercise.id]??true} onChange={e=>setCompleted(current=>({...current,[currentExercise.id]:e.target.checked}))}/><span>Realizado</span></label></div><h1>{currentExercise.name}</h1>{currentExercise.notes?<div className="planned-note">ðŸ“Œ {currentExercise.notes}</div>:null}{previous?<div className="previous-load"><span>Ãšltima execuÃ§Ã£o</span><strong>{previous.sets&&previous.reps?`${previous.sets}Ã—${previous.reps}`:""}{previous.load?` Â· ${previous.load}`:""}</strong><small>{formatDate(previous.date)}</small></div>:<div className="previous-load muted">Sem execuÃ§Ã£o anterior encontrada.</div>}<div className="planned-fields lesson-fields"><label>SÃ©ries<input value={currentExercise.sets} onChange={e=>updateExercise(currentExercise.id,{sets:e.target.value})}/></label><label>RepetiÃ§Ãµes<input value={currentExercise.reps} onChange={e=>updateExercise(currentExercise.id,{reps:e.target.value})}/></label><label>Carga<input value={currentExercise.load} onChange={e=>updateExercise(currentExercise.id,{load:e.target.value})}/></label></div><label className="lesson-exercise-note">ObservaÃ§Ã£o de hoje<input value={currentExercise.notes||""} onChange={e=>updateExercise(currentExercise.id,{notes:e.target.value})} placeholder="Ajuste feito hoje..."/></label><div className="lesson-actions"><button className="secondary" disabled={currentIndex===0} onClick={()=>setCurrentIndex(i=>Math.max(0,i-1))}>â† Anterior</button><button className="primary" onClick={()=>{setCompleted(current=>({...current,[currentExercise.id]:true}));setCurrentIndex(i=>Math.min(exercises.length-1,i+1));}}>{currentIndex===exercises.length-1?"âœ“ Ãšltimo exercÃ­cio":"Concluir e prÃ³ximo â†’"}</button></div></article>
      <button className="secondary" onClick={()=>setLessonMode(false)}>Voltar para ficha completa</button>
    </section></main>;
  }

  return <main className="app-page"><Header title={`${student.name} â€” Treino ${slot}`} back={onBack}/><section className="content narrow"><div className="planned-student-identity"><span>ALUNO</span><strong>{student.name}</strong><small>Treino {slot}</small></div>
    {student.restrictions||student.injuries ? <div className="session-alert"><strong>âš  AtenÃ§Ã£o com {student.name}</strong><span>{[student.restrictions,student.injuries].filter(Boolean).join(" Â· ")}</span></div> : null}<div className="session-mode-banner"><span>ðŸ“‹ Treino {slot} Â· {workoutProtocolLabel(protocol)}</span><strong>{workout?.name||`Treino ${slot}`}</strong><small>{completedCount}/{exercises.length} exercÃ­cios marcados{workout?.notes?` Â· ${workout.notes}`:""}</small><button className="secondary compact-button" onClick={()=>setLessonMode(true)}>â–¶ Modo aula</button></div>
    <div className="session-list">{exercises.map(ex=>{const previous=findPreviousExercise(student,ex.name);return <article className={`session-exercise planned-row ${completed[ex.id]?"is-done":""}`} key={ex.id}>
      ...
      <div className="planned-exercise-main"><div className="planned-title-line"><span className="status-chip">{ex.block||"â€”"}</span><input className="planned-name" value={ex.name} onChange={e=>updateExercise(ex.id,{name:e.target.value})}/></div>{ex.notes?<small className="planned-note-inline">ðŸ“Œ {ex.notes}</small>:null}{previous?<small className="last-load-inline">Ãšltima: {previous.sets&&previous.reps?`${previous.sets}Ã—${previous.reps}`:""}{previous.load?` Â· ${previous.load}`:""} Â· {formatDate(previous.date)}</small>:null}<div className="planned-fields"><input placeholder="SÃ©ries" value={ex.sets} onChange={e=>updateExercise(ex.id,{sets:e.target.value})}/><input placeholder="Reps" value={ex.reps} onChange={e=>updateExercise(ex.id,{reps:e.target.value})}/><input placeholder="Carga" value={ex.load} onChange={e=>updateExercise(ex.id,{load:e.target.value})}/><input placeholder="ObservaÃ§Ã£o de hoje" value={ex.notes||""} onChange={e=>updateExercise(ex.id,{notes:e.target.value})}/></div></div>
    </article>})}</div>
    <div className="panel form-stack"><label>Data<input type="date" value={sessionDate} onChange={e=>setSessionDate(e.target.value)}/></label><label>AlteraÃ§Ãµes / observaÃ§Ãµes<textarea rows={6} value={notes} onChange={e=>setNotes(e.target.value)} placeholder="Ex.: exercÃ­cio substituÃ­do, carga alterada, bloco nÃ£o realizado..."/></label><button className="primary finish-button" disabled={!exercises.some(ex=>ex.name.trim())} onClick={()=>onSave({id:crypto.randomUUID(),date:sessionDate,workoutName:workout?.name||`Treino ${slot}`,workoutId:workout?.id,notes,completedExercises:exercises.filter(ex=>ex.name.trim()),source:"PLANNED",startedAt,finishedAt:new Date().toISOString()})}>âœ“ Treino concluÃ­do â€” salvar no histÃ³rico</button></div>
  </section></main>;
}
...
function FreeSessionScreen({student,onBack,onSave}:{student:Student;onBack:()=>void;onSave:(session:Session)=>void}) {
  ...
  if(!SpeechRecognition){
    alert("O reconhecimento de voz nÃ£o estÃ¡ disponÃ­vel neste navegador. Use o campo de texto.");
    return;
  }
  ...
  const appendWithoutDuplicate=(current:string,incoming:string)=>{
    ...
    if(normalizedLast && now-lastAt<3500){
      ...
      // Se a nova hipÃ³tese apenas cresceu, acrescenta sÃ³ a parte nova.
      ...
      // Se voltou uma hipÃ³tese menor jÃ¡ contida na anterior, ignora.
    ...
    // Remove sobreposiÃ§Ã£o entre o final jÃ¡ salvo e o comeÃ§o do novo trecho.
  ...
  const startRecognition=()=>{
    ...
    // Ãndices jÃ¡ processados nesta instÃ¢ncia do Chrome.
    ...
    recognition.onend=()=>{
      ...
      // Se o navegador parou sozinho, recomeÃ§a sem desligar o modo.
      if(voiceWindow.__dmpShouldListen){
        setListening(true);
  ...
  // Primeiro toque: inicia o modo contÃ­nuo.
  voiceWindow.__dmpShouldListen=true;
  voiceWindow.__dmpLastFinalText="";
  voiceWindow.__dmpLastFinalAt=0;
  startRecognition();
}
  return <main className="app-page"><Header title={`${student.name} â€” Registro rÃ¡pido`} back={onBack}/><section className="content free-session-layout">
    <article className="panel form-stack quick-register-panel">{student.restrictions ? <div className="session-alert"><strong>âš  AtenÃ§Ã£o com {student.name}</strong><span>{student.restrictions}</span></div> : null}<div className="session-mode-banner free"><span>âš¡ Sem precisar de ficha</span><strong>Registre depois da aula</strong><small>Fale ou escreva exatamente como vocÃª costuma me contar o treino.</small></div><label>Data<input type="date" value={sessionDate} onChange={e=>setSessionDate(e.target.value)}/></label><label>Nome / foco da sessÃ£o<input value={focus} onChange={e=>setFocus(e.target.value)} placeholder="Ex.: Peito + core, Full body, MMII..."/></label><label>O que foi feito<textarea rows={10} value={transcript} onChange={e=>setTranscript(e.target.value)} placeholder={'Ex.: Bloco 1: supino reto 4x12 com 18 kg; agachamento goblet 4x15.\nBloco 2: remada baixa 4x12 45 kg; prancha atÃ© a falha.'}/></label><div className="hero-actions"><button className="secondary" onClick={listen}>{listening?"Ouvindo...":"ðŸŽ¤ Falar"}</button><button
  type="button"
  className="primary"
  onClick={()=>{
    if(listening){
...
>
  Organizar para revisÃ£o
</button></div></article>
    <article className="panel"><div className="panel-head"><div><h2>Revise antes de salvar</h2><p className="muted">VocÃª pode corrigir bloco, exercÃ­cio, sÃ©ries, repetiÃ§Ãµes e carga.</p></div><button className="secondary" onClick={()=>setExercises(current=>[...current,{id:crypto.randomUUID(),block:"",name:"",sets:"",reps:"",load:""}])}>+ ExercÃ­cio</button></div>{exercises.length? <div className="review-list">{exercises.map(ex=><div className="review-row enhanced" key={ex.id}><input placeholder="Bloco" value={ex.block||""} onChange={e=>updateExercise(ex.id,{block:e.target.value})}/><input className="review-name" placeholder="ExercÃ­cio" value={ex.name} onChange={e=>updateExercise(ex.id,{name:e.target.value})}/><input placeholder="SÃ©ries" value={ex.sets} onChange={e=>updateExercise(ex.id,{sets:e.target.value})}/><input placeholder="Reps" value={ex.reps} onChange={e=>updateExercise(ex.id,{reps:e.target.value})}/><input placeholder="Carga" value={ex.load} onChange={e=>updateExercise(ex.id,{load:e.target.value})}/><button className="danger-link" onClick={()=>setExercises(current=>current.filter(item=>item.id!==ex.id))}>Ã—</button></div>)}</div>:<div className="empty-review"><strong>Ainda nÃ£o organizado</strong><span>Escreva ou fale o treino e toque em â€œOrganizar para revisÃ£oâ€.</span></div>}<label className="form-stack">ObservaÃ§Ãµes / prÃ³ximos ajustes<textarea rows={4} value={notes} onChange={e=>setNotes(e.target.value)} placeholder="Ex.: nÃ£o fizemos bloco 4; trocar exercÃ­cio no prÃ³ximo treino..."/></label><button className="primary finish-button" disabled={!exercises.some(ex=>ex.name.trim())} onClick={()=>onSave({id:crypto.randomUUID(),date:sessionDate,workoutName:focus||"Treino realizado",notes,completedExercises:exercises.filter(ex=>ex.name.trim()),source:"FREE"})}>âœ“ Salvar no histÃ³rico</button></article>
  </section></main>;
}
...
function AttendanceSessionScreen({student,onBack,onSave}:{student:Student;onBack:()=>void;onSave:(session:Session)=>void}) {
  const [sessionDate,setSessionDate]=useState(today());
  const [notes,setNotes]=useState("");
  return <main className="app-page"><Header title={`${student.name} â€” PresenÃ§a`} back={onBack}/><section className="content narrow">
    {student.restrictions ? <div className="session-alert"><strong>âš  AtenÃ§Ã£o com {student.name}</strong><span>{student.restrictions}</span></div> : null}
    <article className="panel attendance-panel"><div className="attendance-hero"><div className="attendance-check">âœ“</div><div><span>Registro simples</span><h1>Marcar que treinou</h1><p>Use quando vocÃª quer registrar somente a presenÃ§a, sem detalhar exercÃ­cios.</p></div></div><div className="form-stack"><label>Data<input type="date" value={sessionDate} onChange={e=>setSessionDate(e.target.value)}/></label><label>ObservaÃ§Ã£o opcional<textarea rows={5} value={notes} onChange={e=>setNotes(e.target.value)} placeholder="Ex.: sessÃ£o curta, chegou atrasado, jogou tÃªnis depois..."/></label><button className="primary finish-button" onClick={()=>onSave({id:crypto.randomUUID(),date:sessionDate,workoutName:"PresenÃ§a",notes,completedExercises:[],source:"ATTENDANCE"})}>âœ“ Salvar presenÃ§a</button></div></article>
  </section></main>;
}
...
function parseGalileuPdfText(raw:string):GalileuPdfImport{
  ...
  const dateMatch=text.match(/(\d{2}\/\d{2}\/\d{4})\s+(?:as|Ã s)/i);
...
function AssessmentForm({onClose,onSave}:{onClose:()=>void;onSave:(assessment:Assessment)=>void}) {
  ...
  function organizeAssessmentText(){
    ...
    if(!entries.length){setAssessmentTextMessage("NÃ£o encontrei dados reconhecÃ­veis nesse texto.");return;}
  ...
  return <div className="modal-backdrop"><section className="modal assessment-modal"><div className="modal-head"><div><h2>Nova avaliaÃ§Ã£o</h2><p className="muted">Cole os dados da avaliaÃ§Ã£o, organize automaticamente e revise antes de salvar.</p></div><button className="text-button" onClick={onClose}>Fechar</button></div><div className="assessment-pdf-box assessment-text-import"><div><strong>Importar avaliaÃ§Ã£o por texto</strong><small>Cole abaixo os dados organizados da avaliaÃ§Ã£o e o DMP preencherÃ¡ os campos automaticamente para vocÃª revisar.</small></div><textarea rows={10} value={assessmentText} onChange={e=>setAssessmentText(e.target.value)} placeholder={"Cole aqui os dados da avaliaÃ§Ã£o\n\nEx.: Peso: 60,5 kg\nMassa magra kg: 43,5\nMassa magra %: 71,8\nÃ‚ngulo de fase: 6,5"}></textarea><button type="button" className="primary" onClick={organizeAssessmentText}>Organizar dados</button>{assessmentTextMessage?<p>{assessmentTextMessage}</p>:null}</div><div className="assessment-form-section"><h3>ComposiÃ§Ã£o corporal</h3><div className="form-grid"><label>Data<input type="date" value={values.date} onChange={e=>setValues({...values,date:e.target.value})}/></label><label>Peso (kg)<input type="number" step="0.1" value={values.weight} onChange={e=>setValues({...values,weight:e.target.value})}/></label><label>Altura (cm)<input type="number" step="0.1" value={values.height} onChange={e=>setValues({...values,height:e.target.value})}/></label><label>IMC<input type="number" step="0.1" value={values.bmi} onChange={e=>setValues({...values,bmi:e.target.value})} placeholder={calculatedBmi?calculatedBmi.toFixed(1):"Calculado se vazio"}/></label><label>Gordura corporal (%)<input type="number" step="0.1" value={values.bodyFatPercent} onChange={e=>setValues({...values,bodyFatPercent:e.target.value})}/></label><label>Massa de gordura (kg)<input type="number" step="0.1" value={values.fatMass} onChange={e=>setValues({...values,fatMass:e.target.value})}/></label><label>Massa magra (kg)<input type="number" step="0.1" value={values.leanMass} onChange={e=>setValues({...values,leanMass:e.target.value})}/></label><label>Massa magra (%)<input type="number" step="0.1" value={values.leanMassPercent} onChange={e=>setValues({...values,leanMassPercent:e.target.value})} placeholder={calculatedLeanPercent?calculatedLeanPercent.toFixed(1):"Calculado se vazio"}/></label><label>Ãgua corporal (%)<input type="number" step="0.1" value={values.waterPercent} onChange={e=>setValues({...values,waterPercent:e.target.value})}/></label><label>Ãgua corporal total (L)<input type="number" step="0.1" value={values.totalBodyWaterLiters} onChange={e=>setValues({...values,totalBodyWaterLiters:e.target.value})}/></label><label>Ãndice de hidrataÃ§Ã£o<input type="number" step="0.1" value={values.hydrationIndex} onChange={e=>setValues({...values,hydrationIndex:e.target.value})}/></label><label>Ãgua na massa magra (%)<input type="number" step="0.1" value={values.waterLeanPercent} onChange={e=>setValues({...values,waterLeanPercent:e.target.value})}/></label><label>Ãgua intracelular (L)<input type="number" step="0.1" value={values.intracellularWaterLiters} onChange={e=>setValues({...values,intracellularWaterLiters:e.target.value})}/></label><label>Ãgua extracelular (L)<input type="number" step="0.1" value={values.extracellularWaterLiters} onChange={e=>setValues({...values,extracellularWaterLiters:e.target.value})}/></label><label>Ãgua intracelular (%)<input type="number" step="0.1" value={values.intracellularWaterPercent} onChange={e=>setValues({...values,intracellularWaterPercent:e.target.value})}/></label><label>Massa muscular (kg)<input type="number" step="0.1" value={values.muscleMass} onChange={e=>setValues({...values,muscleMass:e.target.value})}/></label><label>Massa muscular (%)<input type="number" step="0.1" value={values.muscleMassPercent} onChange={e=>setValues({...values,muscleMassPercent:e.target.value})}/></label><label>RazÃ£o mÃºsculo/gordura<input type="number" step="0.1" value={values.muscleFatRatio} onChange={e=>setValues({...values,muscleFatRatio:e.target.value})}/></label><label>Metabolismo basal (kcal)<input type="number" step="1" value={values.basalMetabolicRate} onChange={e=>setValues({...values,basalMetabolicRate:e.target.value})}/></label><label>Ã‚ngulo de fase (Â°)<input type="number" step="0.1" value={values.phaseAngle} onChange={e=>setValues({...values,phaseAngle:e.target.value})}/></label><label>Idade celular<input type="number" step="1" value={values.cellularAge} onChange={e=>setValues({...values,cellularAge:e.target.value})}/></label><label>Gordura visceral<input type="number" step="0.1" value={values.visceralFat} onChange={e=>setValues({...values,visceralFat:e.target.value})}/></label><label>Massa celular (kg)<input type="number" step="0.1" value={values.bodyCellMass} onChange={e=>setValues({...values,bodyCellMass:e.target.value})}/></label></div></div><div className="assessment-form-section"><h3>Perimetria</h3><div className="form-grid">{measurementFields.map(([key,label])=><label key={key}>{label} (cm)<input type="number" step="0.1" value={values.measurements[key]||""} onChange={e=>setValues({...values,measurements:{...values.measurements,[key]:e.target.value}})}/></label>)}</div></div><div className="assessment-form-section"><h3>Registro complementar</h3><div className="form-grid"><label className="full">Link da avaliaÃ§Ã£o / Galileu<input type="url" value={values.sourceUrl} onChange={e=>setValues({...values,sourceUrl:e.target.value})} placeholder="Opcional: cole aqui o endereÃ§o da pÃ¡gina da avaliaÃ§Ã£o"/></label>{values.sourceFileName?<div className="full assessment-source-file"><strong>PDF importado:</strong> {values.sourceFileName}</div>:null}<label className="full">Fotos / relatÃ³rio<input type="file" accept="image/*" multiple onChange={e=>photos(e.target.files)}/><small>AtÃ© 4 imagens. Revise antes de salvar.</small></label><label className="full">ObservaÃ§Ãµes<textarea rows={4} value={values.notes} onChange={e=>setValues({...values,notes:e.target.value})}/></label><button className="primary full" disabled={pdfImporting} onClick={()=>onSave({id:crypto.randomUUID(),date:values.date,weight:num(values.weight),height:num(values.height),bodyFatPercent:num(values.bodyFatPercent),fatMass:num(values.fatMass),leanMass:num(values.leanMass),leanMassPercent:num(values.leanMassPercent),bmi:num(values.bmi),waterPercent:num(values.waterPercent),muscleMass:num(values.muscleMass),muscleMassPercent:num(values.muscleMassPercent),basalMetabolicRate:num(values.basalMetabolicRate),phaseAngle:num(values.phaseAngle),visceralFat:num(values.visceralFat),bodyCellMass:num(values.bodyCellMass),hydrationIndex:num(values.hydrationIndex),totalBodyWaterLiters:num(values.totalBodyWaterLiters),waterLeanPercent:num(values.waterLeanPercent),intracellularWaterLiters:num(values.intracellularWaterLiters),extracellularWaterLiters:num(values.extracellularWaterLiters),intracellularWaterPercent:num(values.intracellularWaterPercent),muscleFatRatio:num(values.muscleFatRatio),cellularAge:num(values.cellularAge),sourceUrl:values.sourceUrl.trim(),sourceFileName:values.sourceFileName,measurements:values.measurements,notes:values.notes,photos:values.photos})}>Salvar avaliaÃ§Ã£o</button></div></div></section></div>;
}
...
function parseTranscript(text:string):Exercise[] {
  ...
  for(const raw of parts){
    ...
    if(!part) continue;
    const match=part.match(/(\d+)\s*[xÃ—]\s*([\dâ€“â€”-]+|falha|f)/i);
    const load=part.match(/(?:com|carga|-)?\s*([\d,.]+\s*(?:kg|quilos?|cada lado|kg\/lado))/i);
    let name=part.replace(/\d+\s*[xÃ—]\s*([\dâ€“â€”-]+|falha|f)/i,"").replace(/(?:com|carga|-)?\s*[\d,.]+\s*(?:kg|quilos?|cada lado|kg\/lado)/i,"").trim().replace(/^[,.-]+|[,.-]+$/g,"").trim();
    exercises.push({id:crypto.randomUUID(),block:currentBlock,name:name||`ExercÃ­cio ${exercises.length+1}`,sets:match?.[1]||"",reps:match?.[2]||"",load:load?.[1]||""});
  }
  return exercises;
}
...
const WORKOUT_TEMPLATES:WorkoutTemplate[]=[
  {id:"b7",label:"B7",description:"4 blocos com 2 exercÃ­cios",protocol:"B7",sequenceSize:2,exercises:blankTemplateExercises(8,2)},
  {id:"biset",label:"Bi-set",description:"3 blocos com 2 exercÃ­cios",protocol:"BISET",sequenceSize:2,exercises:blankTemplateExercises(6,2)},
  {id:"circuit",label:"Circuito",description:"6 exercÃ­cios em sequÃªncia",protocol:"CIRCUIT",sequenceSize:6,exercises:blankTemplateExercises(6,6)},
  {id:"travel",label:"Viagem / academia",description:"Ficha geral fÃ¡cil de adaptar",protocol:"CONVENTIONAL",sequenceSize:1,notes:"Treino para realizar durante a viagem. Ajustar cargas conforme os equipamentos disponÃ­veis.",exercises:["Agachamento livre com halteres","Puxada frontal","Supino com halteres","Cadeira extensora","Remada baixa","Mesa ou cadeira flexora"].map(name=>({name,sets:"3",reps:"12"}))},
  {id:"bodyweight",label:"Sem equipamentos",description:"Treino usando o peso corporal",protocol:"CIRCUIT",sequenceSize:6,notes:"Executar respeitando limitaÃ§Ãµes e espaÃ§o disponÃ­vel.",exercises:["Agachamento livre","AvanÃ§o alternado","FlexÃ£o de braÃ§o","ElevaÃ§Ã£o de quadril","Prancha","Polichinelo"].map(name=>({name,sets:"3",reps:"12"}))}
];
...
function workoutValidityInfo(student:Student,workout:Workout){
  ...
  if(mode==="PERIOD"){
    ...
    if(!start)return {status:"NONE" as WorkoutValidityStatus,label:"Sem data de inÃ­cio",remaining:null as number|null,endDate:"",completed:0};
    ...
    if(daysLeft<=6)return {status:"RENEW" as WorkoutValidityStatus,label:"Ãšltima semana",remaining:daysLeft+1,endDate,completed:0};
  ...
  if(remaining<=2)return {status:"RENEW" as WorkoutValidityStatus,label:"Faltam "+remaining+" sessÃ£o"+(remaining===1?"":"Ãµes"),remaining,endDate:"",completed};
...
function protocolDescription(protocol:WorkoutProtocol,size:number){
  if(protocol==="CONVENTIONAL")return size===1?"ExercÃ­cios executados individualmente. VocÃª ainda pode aumentar a sequÃªncia se quiser.":`Convencional organizado em grupos de ${size}.`;
  if(protocol==="BISET")return `Bi-set: o editor agrupa ${size} exercÃ­cios por sequÃªncia. O padrÃ£o Ã© 2, mas vocÃª pode ajustar.`;
  if(protocol==="TRISET")return `Tri-set: o editor agrupa ${size} exercÃ­cios por sequÃªncia. O padrÃ£o Ã© 3.`;
  if(protocol==="B7")return `B7: protocolo identificado na ficha e organizado em sequÃªncias de ${size} exercÃ­cios. VocÃª pode ajustar esse nÃºmero.`;
  if(protocol==="CIRCUIT")return `Circuito com ${size} exercÃ­cios por sequÃªncia. Ajuste a quantidade conforme a aula.`;
  return `Misto / personalizado com ${size} exercÃ­cios por sequÃªncia. Use a coluna Seq. para mudar blocos especÃ­ficos.`;
}

function openWorkoutSharePreview(student:Student,workout:Workout){
  const popup=window.open("","_blank","width=900,height=760");
  if(!popup){alert("O navegador bloqueou a prÃ©via. Permita pop-ups para o DMP e tente novamente.");return;}
  const esc=(value:string)=>value.replace(/[&<>"']/g,char=>({"&":"&amp;","<":"&lt;",">":"&gt;",'"':"&quot;","'":"&#039;"}[char]||char));
  const groups=new Map<string,Exercise[]>();
  workout.exercises.forEach((exercise,index)=>{const block=exercise.block?.trim()||sequenceBlockLabel(workout.protocol||"CONVENTIONAL",index,workout.sequenceSize||defaultSequenceSize(workout.protocol||"CONVENTIONAL"))||"SequÃªncia";groups.set(block,[...(groups.get(block)||[]),exercise]);});
  const blocks=[...groups.entries()].map(([block,items])=>`<section><h3>${esc(block)}</h3>${items.map((ex,i)=>`<div class="exercise"><b>${i+1}. ${esc(ex.name)}</b><span>${esc(ex.sets||"â€”")} Ã— ${esc(ex.reps||"â€”")}${ex.load?` Â· ${esc(ex.load)}`:""}</span>${ex.notes?`<small>${esc(ex.notes)}</small>`:""}</div>`).join("")}</section>`).join("");
  const workoutNotes=workout.notes?.trim()?`<aside class="workout-notes"><strong>OrientaÃ§Ãµes do treino</strong><p>${esc(workout.notes.trim())}</p></aside>`:"";
...
function buildExerciseCatalog(students:Student[],templates:PersonalWorkoutTemplate[]=[]){
  ...
  for(const student of students){
    // O catÃ¡logo deve refletir exercÃ­cios prescritos, nÃ£o transcriÃ§Ãµes do histÃ³rico de aulas.
    // Isso evita sugestÃµes duplicadas com sÃ©ries, repetiÃ§Ãµes ou cargas gravadas no nome.
...
async function downloadWorkoutJpg(student:Student,workout:Workout){
  ...
  workout.exercises.forEach((ex,index)=>{const block=ex.block?.trim()||sequenceBlockLabel(workout.protocol||"CONVENTIONAL",index,workout.sequenceSize||defaultSequenceSize(workout.protocol||"CONVENTIONAL"))||"SequÃªncia";groups.set(block,[...(groups.get(block)||[]),ex]);});
...
function matchCalendarEvents(events:CalendarEvent[],students:Student[]):CalendarEvent[]{
  ...
  return events.map(event=>{
    ...
    const matches=active.filter(student=>{
      ...
      // Aceita qualquer parte Ãºnica do nome, inclusive sobrenomes usados na agenda.
      if(parts.some(part=>{
      ...
      // Aceita abreviaÃ§Ãµes como "Gra" somente quando apontam para um Ãºnico aluno ativo.
...
function formatCalendarTime(event:CalendarEvent){if(event.allDay)return"Dia todo";if(!event.start)return"â€”";const date=new Date(event.start);return date.toLocaleTimeString("pt-BR",{hour:"2-digit",minute:"2-digit"});}
...
function agendaRangeLabel(value:AgendaRange){return value==="day"?"Dia":value==="week"?"Semana":value==="month"?"MÃªs":value==="year"?"Ano":"Lista";}
...
function agendaRangeTitle(range:AgendaRange){return range==="day"?"Compromissos do dia":range==="week"?"Semana de trabalho":range==="month"?"CalendÃ¡rio mensal":range==="year"?"CalendÃ¡rio anual":"PrÃ³ximos compromissos";}
...
function calendarEventStatus(event:CalendarEvent){
  ...
  if(diff < -60)return"HorÃ¡rio passou";
  if(diff < 0)return"Agora";
  if(diff <= 30)return`Em ${diff} min`;
  return"Agendado";
}

function sessionSourceLabel(session:Session){return session.source==="ATTENDANCE"?"PresenÃ§a":session.source==="IMPORTED"?"Importado":session.source==="FREE"?"Treino registrado":"Ficha concluÃ­da";}
...
function organizeQuickTranscript(rawText:string): Exercise[] {
  ...
  let normalized=text.replace(
    /\b(um|uma|dois|duas|tres|trÃªs|quatro|cinco|seis|sete|oito|nove|dez|onze|doze|treze|quatorze|catorze|quinze|dezesseis|dezessete|dezoito|dezenove|vinte)\b/gi,
  ...
  normalized=normalized
    ...
    .replace(/\b(?:prÃ³ximo exercÃ­cio|proximo exercicio|novo exercÃ­cio|novo exercicio)\b/gi,"\n")
    .replace(/\n{2,}/g,"\n");
  ...
  for(const rawLine of normalized.split(/\n+/)){
    ...
    const blockMatch=line.match(/^\s*bloco\s+(\d+)\s*[:\-â€“â€”]?\s*(.*)$/i);
    if(blockMatch){
    ...
    const prescription=line.match(
      /\b(\d+)\s*(?:s[eÃ©]ries?\s*(?:de\s*)?|[xÃ—]\s*|\s+de\s+)(\d+|falha)\b/i
    );
    ...
    if(!prescription){
      output.push({
        id:crypto.randomUUID(),
        block:currentBlock,
        name:line.replace(/^[\s,;:.\-â€“â€”â€¢]+|[\s,;:.\-â€“â€”â€¢]+$/g,"").trim(),
        sets:"",
        reps:"",
        load:"",
        notes:""
    ...
    output.push({
      id:crypto.randomUUID(),
      block:currentBlock,
      name:name||`ExercÃ­cio ${output.length+1}`,
      sets:prescription[1]||"",
      reps:prescription[2]||"",
      load,
      notes
...
function formatDate(value:string){if(!value)return"â€”";const [year,month,day]=value.split("-");return `${day}/${month}/${year}`;}
...
function formatMonths(months:number){const years=Math.floor(months/12);const rest=months%12;return [years?`${years} ano${years>1?"s":""}`:"",rest?`${rest} ${rest===1?"mÃªs":"meses"}`:""].filter(Boolean).join(" e ")||"menos de 1 mÃªs";}
function displayNumber(value:number|null|undefined,suffix:string){return value===null||value===undefined?"â€”":`${Number(value).toLocaleString("pt-BR",{maximumFractionDigits:1})} ${suffix}`;}
function tabLabel(tab:StudentTab){return({summary:"Resumo",workouts:"Treinos",history:"HistÃ³rico",assessments:"AvaliaÃ§Ãµes",files:"Arquivos"})[tab];}
