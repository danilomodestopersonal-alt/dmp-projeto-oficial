...
fetch("/api/google/status").then(r=>r.json()).then(setCalendarStatus).catch(()=>{});
  ...
  async function registerAbsence(student:Student,event:CalendarEvent){
    const absence:Session={id:crypto.randomUUID(),date:today(),workoutName:"Ausência",notes:`Ausência informada para ${event.summary}.`,completedExercises:[],source:"ABSENCE",finishedAt:new Date().toISOString()};
    const remaining=getCalendarEventStudents(event,students).filter(item=>item.id!==student.id);
    const nextSummary=remaining.length?remaining.map(item=>calendarStudentDisplayName(item)).join(" "):event.summary.replace(new RegExp(student.name.split(" ")[0],"i"),"").replace(/\s{2,}/g," ").trim()||"Horário disponível";
    const response=await fetch("/api/google/events",{method:"PATCH",headers:{"content-type":"application/json"},body:JSON.stringify({id:event.id,summary:nextSummary,description:event.description||"",location:event.location||"",start:event.start,end:event.end})});
    ...
    setCalendarEvents(current=>current.map(item=>item.id===event.id?{...item,summary:nextSummary,matchedStudentIds:remaining.map(value=>value.id)}:item));
  }
        ...
        <div className="dashboard-main">
          {view === "today" ? <>
            ...
            <div className="home-desktop-layout"><section className="dashboard-content home-main-content">
              ...
              <TodayHighlights events={calendarEvents.filter(event=>calendarEventDate(event)===todayKey)} notes={notes} onAgenda={(date)=>{setCalendarAnchor(date);setView("agenda");}} onKids={()=>setView("kids")} onNotes={()=>document.querySelector(".notes-panel")?.scrollIntoView({behavior:"smooth"})}/>
...
function TodayHighlights({events,notes,onAgenda,onKids,onNotes}:{events:CalendarEvent[];notes:DmpNote[];onAgenda:(date:string)=>void;onKids:()=>void;onNotes:()=>void}){
  const now=new Date();
  const timed=events.filter(event=>!event.allDay).sort((a,b)=>a.start.localeCompare(b.start));
  const finished=timed.filter(event=>new Date(event.end).getTime()<=now.getTime()).length;
  const remaining=Math.max(0,timed.length-finished);
  const progress=timed.length?Math.round((finished/timed.length)*100):0;
  ...
  return <div className="today-highlight-grid"><MiniMonthCalendar onSelect={onAgenda}/><button className="today-highlight-card" onClick={()=>onAgenda(today())}><span className="today-highlight-icon">📊</span><div><strong>Resumo do dia</strong><small>{timed.length} aulas · {finished} realizadas · {remaining} restantes</small><i><b style={{width:`${progress}%`}}/></i></div><em>›</em></button><button className="today-highlight-card" onClick={onKids}><span className="today-highlight-icon">🎾</span><div><strong>Aulas Kids hoje</strong><small>{kids.length?kids.slice(0,3).map(({event,kids:item})=>`${formatCalendarTime(event)} · ${kidsCategoryName(item.category)}`).join("  •  "):"Nenhuma aula Kids hoje"}</small></div><em>›</em></button><button className="today-highlight-card" onClick={onNotes}><span className="today-highlight-icon">🗒️</span><div><strong>Recados pendentes</strong><small>{pending.length?`${pending.length} pendente${pending.length===1?"":"s"} · ${pending.slice(0,2).map(note=>note.text).join(" • ")}`:"Nenhum recado pendente"}</small></div><em>›</em></button></div>;
}
function MiniMonthCalendar({onSelect}:{onSelect:(date:string)=>void}){
  const now=new Date();const year=now.getFullYear();const month=now.getMonth();const first=new Date(year,month,1).getDay();const days=new Date(year,month+1,0).getDate();
  return <article className="mini-month"><button onClick={()=>onSelect(today())}><strong>{now.toLocaleDateString("pt-BR",{month:"long"})}</strong><span>abrir agenda ›</span></button><div className="mini-month-week"><b>D</b><b>S</b><b>T</b><b>Q</b><b>Q</b><b>S</b><b>S</b></div><div className="mini-month-days">{Array.from({length:first},(_,index)=><i key={`e-${index}`}/>)}{Array.from({length:days},(_,index)=>{const day=index+1;const value=`${year}-${String(month+1).padStart(2,"0")}-${String(day).padStart(2,"0")}`;return <button key={day} className={day===now.getDate()?"today":""} onClick={()=>onSelect(value)}>{day}</button>;})}</div></article>;
}
...
function DesktopAgendaRail({events,students,onOpenAgenda,onOpenStudent}:{events:CalendarEvent[];students:Student[];onOpenAgenda:(date:string)=>void;onOpenStudent:(id:string)=>void}){
  const upcoming=events.slice().sort((a,b)=>a.start.localeCompare(b.start));
  return <aside className="desktop-agenda-rail"><div className="agenda-rail-head"><span>Próximos compromissos</span><button onClick={()=>onOpenAgenda(today())}>Agenda completa ›</button></div><div className="agenda-rail-scroll">{upcoming.length?upcoming.map(event=>{const people=getCalendarEventStudents(event,students);return <article className={`agenda-rail-event ${kidsCalendarRequest(event)?"kids":""}`} key={event.id}><button onClick={()=>onOpenAgenda(calendarEventDate(event))}><small>{formatRailDate(event)} · {formatCalendarTime(event)}</small><strong>{event.summary}</strong></button>{people.length?<div>{people.map(student=><button key={student.id} onClick={()=>onOpenStudent(student.id)}>{student.name}</button>)}</div>:null}</article>}):<p>Nenhum compromisso carregado.</p>}</div></aside>;
}
