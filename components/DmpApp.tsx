...
function AssessmentForm({onClose,onSave}:{onClose:()=>void;onSave:(assessment:Assessment)=>void}) {
  ...
  async function importPdf(files:FileList|null){const file=files?.[0];if(!file)return;if(!file.name.toLowerCase().endsWith(".pdf")){setPdfImportMessage("Selecione um PDF do Galileu.");return;}setPdfImporting(true);setPdfImportMessage("Lendo o PDF do Galileu… isso pode levar alguns segundos.");try{const data=await readGalileuPdf(file);const entries=Object.entries(data).filter(([,value])=>value!==undefined&&value!==null);setValues((current:any)=>{const next={...current,sourceFileName:file.name};for(const [key,value] of entries)next[key]=typeof value==="number"?String(Math.round(value*10)/10):value;return next;});setPdfImportMessage(`${entries.length} dados identificados no PDF. Revise os campos abaixo antes de salvar.`);}catch(error){console.error(error);setPdfImportMessage("Não consegui ler este PDF automaticamente. Você ainda pode preencher os campos manualmente.");}finally{setPdfImporting(false);}}
