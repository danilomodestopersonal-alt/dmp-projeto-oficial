...
fetch("/api/google/status").then(r=>r.json()).then(setCalendarStatus).catch(()=>{});
  ...
  async function registerAbsence(student:Student,event:CalendarEvent){
    const absence:Session={id:crypto.randomUUID(),date:calendarEventDate(event),workoutName:"Ausência",notes:`Ausência informada para ${event.summary}.`,completedExercises:[],source:"ABSENCE",finishedAt:new Date().toISOString()};
      ...
      <main className="dashboard-shell">
        ...
        <div className="dashboard-main">
          {view === "today" ? <>
            ...
            <button className="mobile-quick-launch" onClick={()=>setShowMobileActions(true)} aria-label="Abrir ações rápidas">＋</button>
...
function DesktopAgendaRail({events,students,onOpenAgenda,onOpenStudent}:{events:CalendarEvent[];students:Student[];onOpenAgenda:(date:string)=>void;onOpenStudent:(id:string)=>void}){
  ...
  let previousDate="";
  return <aside className="desktop-agenda-rail"><div className="agenda-rail-head"><div><span>📅</span><strong>Próximos compromissos</strong></div><button onClick={()=>onOpenAgenda(today())}>Abrir agenda completa <b>›</b></button></div><div className="agenda-rail-scroll">{upcoming.length?upcoming.map(event=>{const people=getCalendarEventStudents(event,students);const date=calendarEventDate(event);const changed=date!==previousDate;previousDate=date;return <section className="agenda-rail-group" key={event.id}>{changed?<div className="agenda-rail-day"><span>{date===today()?"Hoje":"Dia seguinte"}</span><strong>{new Date(`${date}T12:00:00`).toLocaleDateString("pt-BR",{weekday:"short",day:"2-digit",month:"short"})}</strong></div>:null}<article className={`agenda-rail-event ${kidsCalendarRequest(event)?"kids":""}`}><button onClick={()=>onOpenAgenda(date)}><small>{formatCalendarTime(event)}</small><strong>{event.summary}</strong></button>{people.length?<div>{people.map(student=><button key={student.id} onClick={()=>onOpenStudent(student.id)}>{student.name}</button>)}</div>:null}</article></section>}):<p>Nenhum compromisso carregado.</p>}</div></aside>;
}
