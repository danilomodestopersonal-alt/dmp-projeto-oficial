...
import {KidsReplacementOperationSummary} from "@/components/kids/KidsReplacementBalance";
...
const [cloudWritable, setCloudWritable] = useState(false);
  ...
  const [homeKidsData,setHomeKidsData]=useState<KidsData|null>(null);
...
fetch("/api/google/status")
  ...
  useEffect(()=>{
    ...
    fetch("/api/kids",{cache:"no-store"})
      ...
      .then(payload=>{
        if(cancelled)return;
        const rawData=payload.data as KidsData|null;
        if(!rawData){
          setHomeMonthKidsCount(0);
          setHomeKidsData(null);
          return;
        }
        const data=normalizeKidsData(rawData);
        setHomeKidsData(data);
        ...
        <div className="dashboard-main">
          {view === "today" ? <>
            ...
            <div className="home-desktop-layout"><section className="dashboard-content home-main-content">
              ...
              <KidsReplacementOperationSummary data={homeKidsData} compact onOpen={()=>{setKidsLessonRequest(null);setKidsStudentRequest(null);setKidsEntryKey(current=>current+1);setView("kids");}} />
