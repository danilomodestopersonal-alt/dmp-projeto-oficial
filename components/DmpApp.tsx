...
type DmpProfileSettings={birthdayImage?:string;birthdayMessage?:string};
function SettingsCenter(){
  const [profile,setProfile]=useState<DmpProfileSettings>({birthdayMessage:"Parabéns pelo seu dia! 🎉 Desejo muita saúde, felicidade e um excelente novo ciclo. Abraço!"});
...
function AssessmentTrendChart({student}:{student:Student}){const [period,setPeriod]=useState<"ALL"|"3"|"6"|"12">("ALL");const sorted=[...student.assessments].sort((a,b)=>a.date.localeCompare(b.date));const cutoff=period==="ALL"?"":dateOffset(today(),-(Number(period)*30));const data=cutoff?sorted.filter(a=>a.date>=cutoff):sorted;const metrics=[{label:"Peso",key:"weight",unit:"kg"},{label:"Gordura corporal",key:"bodyFatPercent",unit:"%"},{label:"Massa muscular",key:"muscleMass",unit:"kg"},{label:"Massa magra",key:"leanMass",unit:"kg"}] as const;return <section className="assessment-trends"><div className="assessment-trends-head"><div><h3>Histórico visual</h3><p>Evolução de peso e composição corporal.</p></div><select value={period} onChange={e=>setPeriod(e.target.value as any)}><option value="3">3 meses</option><option value="6">6 meses</option><option value="12">1 ano</option><option value="ALL">Todo histórico</option></select></div><div className="assessment-trend-grid">{metrics.map(metric=><MiniTrend key={metric.key} label={metric.label} unit={metric.unit} points={data.map(a=>({date:a.date,value:a[metric.key] as number|null|undefined})).filter((p):p is {date:string;value:number}=>typeof p.value==="number")}/>)}</div></section>}
...
function shareStudentSummary(student:Student){const month=today().slice(0,7),stats=monthStudentStats(student,month),sorted=assessmentSorted(student),latest=sorted[0],previous=sorted[1];const first=student.name.trim().split(/\s+/)[0]||student.name;const parts=[`Olá, ${first}! Segue um resumo do seu acompanhamento no DMP:`,`• Frequência em ${monthLabel(month)}: ${stats.done} sessão${stats.done===1?"":"ões"}${stats.absences?` e ${stats.absences} falta${stats.absences===1?"":"s"}`:""}.`];if(latest){parts.push(`• Última avaliação: ${formatDate(latest.date)}${latest.weight!=null?` · ${latest.weight} kg`:""}${latest.bodyFatPercent!=null?` · ${latest.bodyFatPercent}% de gordura`:""}.`);if(previous){const evol=[];if(latest.weight!=null&&previous.weight!=null)evol.push(`peso ${assessmentDelta(latest.weight,previous.weight," kg")}`);if(latest.bodyFatPercent!=null&&previous.bodyFatPercent!=null)evol.push(`gordura ${assessmentDelta(latest.bodyFatPercent,previous.bodyFatPercent," p.p.")}`);if(latest.muscleMass!=null&&previous.muscleMass!=null)evol.push(`massa muscular ${assessmentDelta(latest.muscleMass,previous.muscleMass," kg")}`);if(evol.length)parts.push(`• Evolução desde ${formatDate(previous.date)}: ${evol.join(" · ")}.`);}}parts.push("Seguimos acompanhando sua evolução! 💪");const text=parts.join("\n");const link=whatsappLink(student.phone);if(link)window.open(`${link}?text=${encodeURIComponent(text)}`,"_blank");else{navigator.clipboard?.writeText(text);alert("Resumo copiado. Cadastre o WhatsApp do aluno para abrir a conversa diretamente.");}}
...
function GlobalSearch({value,onChange,students,events,onStudent,onAgenda}:{value:string;onChange:(value:string)=>void;students:Student[];events:CalendarEvent[];onStudent:(id:string)=>void;onAgenda:(date:string)=>void}){
  ...
  useEffect(()=>{if(!q)return;let cancelled=false;const local=loadFinanceData(financeSeedAugust2026);setFinance(local);fetchFinanceCloud(financeSeedAugust2026).then(data=>{if(!cancelled&&data)setFinance(data);}).catch(()=>{});return()=>{cancelled=true};},[q]);
  ...
  const studentHits=students.map(student=>{const session=student.sessions.find(x=>normalizeName(`${x.date} ${x.workoutName} ${x.focus||""} ${x.notes} ${x.completedExercises.map(e=>e.name).join(" ")}`).includes(q));const assessment=student.assessments.find(a=>normalizeName(`${a.date} ${a.weight??""} ${a.bodyFatPercent??""} ${a.muscleMass??""} ${a.notes}`).includes(q));const base=normalizeName(`${student.name} ${student.phone} ${student.email||""} ${student.goal} ${student.notes} ${student.restrictions} ${student.profession||""}`);if(base.includes(q))return{student,label:"Aluno",detail:student.goal||"Abrir cadastro"};if(session)return{student,label:"Treino / histórico",detail:`${formatDate(session.date)} · ${session.workoutName}`};if(assessment)return{student,label:"Avaliação",detail:`${formatDate(assessment.date)} · ${assessment.weight??"—"} kg`};return null;}).filter(Boolean).slice(0,8) as {student:Student;label:string;detail:string}[];
  ...
  return <div className="global-search global-search-open"><span>⌕</span><input autoFocus value={value} onChange={e=>onChange(e.target.value)} placeholder="Buscar aluno, treino, avaliação, financeiro..."/><div className="global-search-results">{studentHits.map(hit=><button key={`${hit.student.id}-${hit.label}`} onClick={()=>{onChange("");onStudent(hit.student.id)}}><b>{hit.student.name}</b><small>{hit.label} · {hit.detail}</small></button>)}{financeHits.map(item=>{const st=students.find(s=>normalizeName(s.name)===normalizeName(item.studentName));return <button key={`fin-${item.id}`} onClick={()=>{onChange("");if(st)onStudent(st.id)}}><b>{item.studentName}</b><small>Financeiro · {item.competence} · {formatStudentMoney(item.expectedAmount)}</small></button>})}{eventHits.map(event=><button key={`ev-${event.id}`} onClick={()=>{onChange("");onAgenda(calendarEventDate(event))}}><b>{event.summary}</b><small>Agenda · {formatDate(calendarEventDate(event))}</small></button>)}{!studentHits.length&&!financeHits.length&&!eventHits.length?<p>Nenhum resultado encontrado.</p>:null}</div></div>;
}
...
function StudentDashboardComplete({student,onOpen,onReport,onStudentUpdate}:{student:Student;onOpen:(tab:StudentTab)=>void;onReport:()=>void;onStudentUpdate:(student:Student)=>void}) {
  ...
  const shareSummary=()=>shareStudentSummary(student);
