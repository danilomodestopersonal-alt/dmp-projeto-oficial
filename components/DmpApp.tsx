...
type View = "today" | "students" | "workouts-overview" | "history-overview" | "agenda" | "student" | "workout-editor" | "planned-session" | "free-session" | "attendance-session";
...
export default function DmpApp() {
  ...
  if (["today","students","workouts-overview","history-overview","agenda"].includes(view)) {
    ...
    return (
      <main className="dashboard-shell">
        ...
        <div className="dashboard-main">
          ...
          {view === "history-overview" ? <><header className="dashboard-topbar"><div><p className="dashboard-eyebrow">Linha do tempo</p><h1>Histórico</h1><p>Fichas concluídas, treinos livres e presenças em um só lugar.</p></div></header><section className="dashboard-content"><div className="overview-list">{students.flatMap(student=>student.sessions.map(session=>({student,session}))).sort((a,b)=>b.session.date.localeCompare(a.session.date)).slice(0,200).map(({student,session})=><button className="overview-row" key={session.id} onClick={()=>openStudent(student.id)}><span><strong>{formatDate(session.date)} · {student.name}</strong><small>{session.workoutName}{session.notes?` — ${session.notes}`:""}</small></span><span className="status-chip ok">{sessionSourceLabel(session)}</span></button>)}</div></section></> : null}

          {view === "agenda" ? <><header className="dashboard-topbar"><div><p className="dashboard-eyebrow">Central do dia</p><h1>Agenda</h1><p>Seus compromissos do Google Calendar dentro do DMP.</p></div></header><section className="dashboard-content"><CalendarAgenda status={calendarStatus} events={calendarEvents} loading={calendarLoading} students={students} onOpenStudent={openStudent} onStatusChange={setCalendarStatus} /></section></> : null}
        </div>
...
function CalendarAgenda({status,events,loading,students,onOpenStudent,onStatusChange}:{status:{configured:boolean;connected:boolean};events:CalendarEvent[];loading:boolean;students:Student[];onOpenStudent:(id:string)=>void;onStatusChange:(value:{configured:boolean;connected:boolean})=>void}) {
  async function disconnect(){await fetch("/api/google/disconnect",{method:"POST"});onStatusChange({...status,connected:false});}
  return <>
    <section className="panel agenda-connect"><div className="agenda-icon">📅</div><div className="agenda-connect-main"><h2>Google Calendar</h2><p>A agenda permanece sendo a sua agenda oficial do Google. O DMP apenas lê os compromissos e os coloca no seu fluxo de atendimento.</p><div className="agenda-roadmap"><span>✓ Leitura da agenda do dia</span><span>✓ Identificação automática de alunos pelo nome</span><span>✓ Atalho do compromisso para a ficha do aluno</span><span>Próxima fase: criar/editar eventos pelo DMP</span></div></div><div className="agenda-actions">{!status.configured?<span className="status-chip">Configuração pendente</span>:status.connected?<><span className="status-chip ok">Conectado</span><button className="secondary" onClick={disconnect}>Desconectar</button></>:<a className="primary button-link" href="/api/google/auth">Conectar Google</a>}</div></section>
    {!status.configured?<section className="panel setup-panel"><h2>Uma configuração única</h2><p>O código da integração já está pronto. Para ativar, vamos criar duas credenciais no Google Cloud e colocar <code>GOOGLE_CLIENT_ID</code> e <code>GOOGLE_CLIENT_SECRET</code> no projeto. Depois disso, o botão “Conectar Google” passa a funcionar no computador e no celular.</p></section>:null}
    {status.connected?<section className="panel"><div className="panel-head"><div><h2>Compromissos de hoje</h2><p className="muted">{formatLongDate(today())}</p></div><span className="status-chip ok">{events.length} evento{events.length===1?"":"s"}</span></div>{loading?<div className="calendar-empty">Carregando agenda...</div>:events.length?<div className="agenda-event-list">{events.map(event=>{const matched=students.find(s=>s.id===event.matchedStudentId);return <article className="agenda-event-row" key={event.id}><div className="agenda-event-time">{formatCalendarTime(event)}</div><div className="agenda-event-body"><strong>{event.summary}</strong>{event.description?<small>{event.description}</small>:null}{matched?<button className="calendar-student-link" onClick={()=>onOpenStudent(matched.id)}>👤 Abrir {matched.name}</button>:<small className="muted">Sem aluno correspondente no DMP</small>}</div></article>})}</div>:<div className="calendar-empty">Nenhum compromisso encontrado para hoje.</div>}</section>:null}
  </>;
}

function Sidebar({current,onNavigate,logout}:{current:View;onNavigate:(view:View)=>void;logout:()=>void}) {
  const items:{view:View;icon:string;label:string}[]=[{view:"today",icon:"🏠",label:"Hoje"},{view:"students",icon:"👥",label:"Alunos"},{view:"workouts-overview",icon:"🏋️",label:"Treinos"},{view:"history-overview",icon:"📋",label:"Histórico"},{view:"agenda",icon:"📅",label:"Agenda"}];
