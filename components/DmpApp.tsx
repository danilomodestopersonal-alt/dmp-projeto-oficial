...
fetch("/api/google/status").then(r=>r.json()).then(setCalendarStatus).catch(()=>{});
  ...
  if (["today","students","workouts-overview","history-overview","assessments-overview","agenda","finance","performance","data","weather"].includes(view)) {
    ...
    const allAssessments = students.flatMap(student=>student.assessments.map(assessment=>({student,assessment}))).sort((a,b)=>b.assessment.date.localeCompare(a.assessment.date));
