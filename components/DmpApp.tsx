...
</span><span className={latestAssessment?(assessmentExpired?"home-assessment-badge expired":"home-assessment-badge ok"):"home-assessment-badge none"}>{latestAssessment?`📏 ${formatDate(latestAssessment.date)}`:"📏 Sem avaliação"}</span>{workoutEntry?<span className={`home-workout-badge ${workoutValidity?.status.toLowerCase()||"none"}`}><b>Treino {workoutEntry.slot}</b><small>{workoutValidity?.status==="NONE"?`${workoutEntry.workout.exercises.length} exercícios`:workoutValidity?.endDate?`até ${formatDate(workoutValidity.endDate)}`:workoutValidity?.label}</small></span>:<span className="home-workout-badge none"><b>Sem treino</b></span>}<span className={done?"status-chip ok":absent?"status-chip absent":"status-chip waiting"}>{done?"✓ Finalizado":absent?"Ausente":"Pendente"}</span>{!done&&!absent?<span className="calendar-student-actions">{workoutEntry?<button className="primary compact-action" onClick={()=>onStartStudent(student.id,"session")}>▶ Iniciar</button>:null}<button className={workoutEntry?"secondary compact-action":"primary compact-action"} onClick={()=>onStartStudent(student.id,"free")}>✍ Registrar</button><button className="secondary compact-action" onClick={()=>onStartStudent(student.id,"attendance")}>✓ Presença</button><button className="absence-action compact-action" onClick={()=>void onAbsence(student,event)}>Ausência</button></span>:null}</div>})}</div>:null}</div><span className="status-chip">{calendarEventStatus(event)}</span></article>})}</div> : <div className="calendar-empty"><strong>Nenhum compromisso hoje</strong><span>Sua agenda Google está conectada.</span></div>}
  </section>;
}
