...
fetch("/api/google/status").then(r=>r.json()).then(setCalendarStatus).catch(()=>{});
  ...
  function startStudentFlow(studentId:string, mode:"session"|"free"|"attendance") {
    const student = students.find(item => item.id === studentId);
    if (!student) return;
    ...
    return (
      <main className="dashboard-shell">
        ...
        <div className="dashboard-main">
          {view === "today" ? <>
            <header className="dashboard-topbar"><div className="today-heading"><div><p className="dashboard-eyebrow">Sua central do dia</p><h1>{formatWeekday(todayKey)}</h1><p>{formatCalendarDate(todayKey)}</p></div><div className="today-tools"><WeatherWidget onOpen={()=>setView("weather")}/><DigitalClock/></div></div><button className="kids-hero-shortcut" onClick={()=>setView("kids")}><img src="/logo-ctds.png" alt="CT DS Tennis"/><span><small>Gestão das aulas</small><strong>Tênis Kids</strong></span><b>›</b></button></header>
            <section className="dashboard-content">
              <TodayHighlights events={calendarEvents} notes={notes} onAgenda={()=>setView("agenda")} onKids={()=>setView("kids")} onNotes={()=>document.querySelector(".notes-panel")?.scrollIntoView({behavior:"smooth"})}/>
              <CalendarTodayPanel status={calendarStatus} events={calendarEvents} loading={calendarLoading} sync={calendarSync} students={students} todaySessions={todaySessions} onOpenAgenda={() => setView("agenda")} onOpenStudent={openStudent} onStartStudent={startStudentFlow} onOpenKids={openKidsCalendarEvent}/>
          ...
          {view === "agenda" ? <><header className="dashboard-topbar"><div><p className="dashboard-eyebrow">Central do dia</p><h1>Agenda</h1><p>Seus compromissos do Google Calendar dentro do DMP.</p></div></header><section className="dashboard-content"><CalendarAgenda status={calendarStatus} events={calendarEvents} loading={calendarLoading} sync={calendarSync} students={students} onOpenStudent={openStudent} onStartStudent={startStudentFlow} onOpenKids={openKidsCalendarEvent} onStatusChange={setCalendarStatus} onRefresh={()=>void refreshCalendarAutomatic(true)} onNewEvent={()=>setShowGoogleEventForm(true)} /></section></> : null}
...
function CalendarTodayPanel({status,events,loading,sync,students,todaySessions,onOpenAgenda,onOpenStudent,onStartStudent,onOpenKids}:{status:{configured:boolean;connected:boolean};events:CalendarEvent[];loading:boolean;sync:{dailyAt:string;weeklyAt:string;weeklyCount:number};students:Student[];todaySessions:{student:Student;session:Session}[];onOpenAgenda:()=>void;onOpenStudent:(id:string)=>void;onStartStudent:(id:string,mode:"session"|"free"|"attendance")=>void;onOpenKids:(event:CalendarEvent)=>void}) {
...
function CalendarAgenda({status,events,loading,sync,students,onOpenStudent,onStartStudent,onOpenKids,onStatusChange,onRefresh,onNewEvent}:{status:{configured:boolean;connected:boolean};events:CalendarEvent[];loading:boolean;sync:{dailyAt:string;weeklyAt:string;weeklyCount:number};students:Student[];onOpenStudent:(id:string)=>void;onStartStudent:(id:string,mode:"session"|"free"|"attendance")=>void;onOpenKids:(event:CalendarEvent)=>void;onStatusChange:(value:{configured:boolean;connected:boolean})=>void;onRefresh:()=>void;onNewEvent:()=>void}) {
...
function TodayHighlights({events,notes,onAgenda,onKids,onNotes}:{events:CalendarEvent[];notes:DmpNote[];onAgenda:()=>void;onKids:()=>void;onNotes:()=>void}){
  const now=new Date();
