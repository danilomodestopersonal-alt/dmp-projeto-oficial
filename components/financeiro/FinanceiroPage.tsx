      ...
      <section className={`dashboard-content ${styles.content}`}>
        ...
        {tab === "extras" ? (
          <div className={styles.twoColWide}>
            <section className="panel">
              <div className="panel-head"><div><h2>Gastos extras</h2><p className="muted">{summary.extras.length} lançamentos · {money.format(summary.extrasTotal)} realizados.</p></div><button className="primary" disabled={!editable} onClick={() => openAction({ type: "extra-create" })}>+ Novo gasto</button></div>
            ...
            <section className="panel">
              <div className="panel-head"><div><h2>Categorias</h2><p className="muted">Usadas nos gastos extras e no relatório.</p></div><button className="secondary" disabled={!editable} onClick={() => openAction({ type: "category-create" })}>+ Categoria</button></div>
              <div className={styles.categoryList}>{data.categories.map(category => { const used = data.extraExpenses.some(item => item.category === category); return <div className={styles.categoryChip} key={category}><span>{category}</span><button disabled={!editable || used} title={used ? "Categoria em uso" : "Excluir categoria"} onClick={() => { if (window.confirm(`Excluir a categoria “${category}”?`)) dispatch({ type: "CATEGORY_DELETE", name: category, competence }); }}>×</button></div>; })}</div>
            </section>
          </div>
        ) : null}
