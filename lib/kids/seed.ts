...
const seedClasses:SeedClass[]=[
  ...
  {name:"Turma de sábado 10h",weekday:6,startTime:"10:00",category:"PURPLE",students:["Beatriz Roma","Bruna Pallone","Enrico Colbano","Elisa Constanza","Theo Vargas","Caio Vargas","Matheus Leite"]},
];
...
export function createKidsSeed():KidsData{
  ...
  const classes:KidsClass[]=seedClasses.map(item=>({
    id:`kids-class-${slug(item.name)}`,name:item.name,weekday:item.weekday,startTime:item.startTime,endTime:addHour(item.startTime),category:item.category,teacher:"Danilo",active:true,updatedAt:now,
  ...
  for(const date of datesBetween(KIDS_SEMESTER_START,KIDS_SEMESTER_END)){
    ...
    for(const group of classes.filter(item=>item.weekday===weekday)){
      ...
      lessons.push({id:`lesson-${group.id}-${date}`,classId:group.id,date,status:holiday?"HOLIDAY":"SCHEDULED",attendance:{},objective:"",plannedPlan:"",actualPlan:"",notes:holiday?"Aula cancelada por feriado.":"",replacementEligible:false,replacementStatus:"NONE",updatedAt:now});
    }
  }
  return {version:1,semesterStart:KIDS_SEMESTER_START,semesterEnd:KIDS_SEMESTER_END,classes,lessons,updatedAt:now};
}
